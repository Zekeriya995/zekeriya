...
function renderTopCoins(){
  ...
  el.innerHTML=TOP4.map(function(s){var d=T[s];if(!d)return'';var up=d.c>=0;var bg=COL[s]||'#888';
    return'<div class="coin-card '+(up?'up':'dn')+'" onclick="openCoin(\''+s+'\')">'
    +'<div class="coin-card-h"><div class="coin-card-name"><div class="coin-card-ic" style="background:'+bg+'15;color:'+bg+';border:1px solid '+bg+'30">'+TOP4_ICONS[s]+'</div>'+s+'/USDT</div><span style="font-family:var(--fm);font-size:13px;font-weight:800;color:var(--'+(up?'up':'dn')+')">'+(up?'+':'')+d.c.toFixed(2)+'% '+(up?'📈':'📉')+'</span></div>'
    +'<div class="coin-card-body"><div><div class="coin-card-price">'+fP(d.p)+'</div><div class="coin-card-spark">'+mkSpark(s)+'</div></div><div class="coin-card-info"><div>Vol: <b style="color:var(--neon)">'+fmt(d.v)+'</b></div><div>H: <b style="color:var(--up)">'+fP(d.h)+'</b></div><div>L: <b style="color:var(--dn)">'+fP(d.l)+'</b></div></div></div>'
    +'</div>'}).join('')}
...
async function runValidator(){
  ...
  if(tkAge>180000){addVLog('🔴','البيانات قديمة '+Math.round(tkAge/60000)+' دقيقة — يعيد التحميل');issues++;try{await loadTk();lastDataTime=Date.now();fixes++;addVLog('🔧','تم إعادة تحميل البيانات ✅')}catch(e){addVLog('❌','فشل إعادة التحميل')}}
  else if(tkAge>120000){addVLog('🟡','البيانات عمرها '+Math.round(tkAge/60000)+' دقيقة');issues++}
...
async function init(){document.getElementById('sInp').placeholder=t('search_ph');document.getElementById('notifB').dataset.c='0';
  ...
  setInterval(async function(){try{await loadTk();await loadFutures();lastDataTime=Date.now();checkWatchlistAlerts()}catch(e){}},60000);
  ...
  /* Data Validator — every 2 minutes */
  setTimeout(function(){runValidator()},10000);
  setInterval(function(){runValidator()},120000)}
init();
