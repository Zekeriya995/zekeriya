/* ═══════════════════════════════════════════════════════════ */
/*  NEXUS PRO V7 — Complete Engine + Bilingual (AR/EN)        */
/* ═══════════════════════════════════════════════════════════ */
...
let portfolio=JSON.parse(localStorage.getItem('nxp7')||'[]');
...
const TR={
  ...
  brk_label:{ar:'على وشك الانفجار',en:'About to Breakout'},breakout:{ar:'على وشك الانفجار',en:'About to Breakout'},whales:{ar:'تجميع الحيتان',en:'Whale Accumulation'},scanning:{ar:'جاري المسح...',en:'Scanning...'},
  sec_brk_wh:{ar:'💥 على وشك الانفجار & 🐋 تجميع الحيتان',en:'💥 Breakout Radar & 🐋 Whale Accumulation'},sec_movers:{ar:'🔥 الأكثر تحركاً',en:'🔥 TOP MOVERS'},
  all:{ar:'الكل',en:'All'},full_scan:{ar:'مسح شامل',en:'Full Scan'},refresh:{ar:'تحديث',en:'Refresh'},
  ...
  portfolio:{ar:'المحفظة',en:'Portfolio'},risk_calc:{ar:'حاسبة المخاطر',en:'Risk Calculator'},alerts:{ar:'تنبيهات',en:'Alerts'},
  ...
  enter_data:{ar:'ادخل البيانات',en:'Enter data'},instant:{ar:'فوري',en:'Instant'},strong_signal:{ar:'شراء/بيع قوي',en:'Strong buy/sell'},before_unlock:{ar:'قبل الفك',en:'Before unlock'},
  search_ph:{ar:'ابحث عن أي عملة...',en:'Search any coin...'},
  qty_ph:{ar:'الكمية',en:'Quantity'},buy_price_ph:{ar:'سعر الشراء (اختياري)',en:'Buy price (optional)'},
  no_ultra:{ar:'لا ULTRA حالياً — المسح مستمر',en:'No ULTRA signals — scanning continues'},
  no_brk:{ar:'لا عملات على وشك الانفجار',en:'No imminent breakouts'},no_whale:{ar:'لا تجميع حيتان حالياً',en:'No whale accumulation detected'},
  prob:{ar:'احتمال الانفجار',en:'Breakout probability'},
  buy_strong:{ar:'شراء قوي',en:'Strong Buy'},buy:{ar:'شراء',en:'Buy'},sell:{ar:'بيع',en:'Sell'},hold:{ar:'انتظار',en:'Hold'},
  risk_amt:{ar:'💰 مبلغ المخاطرة',en:'💰 Risk Amount'},pos_size:{ar:'📦 حجم الصفقة',en:'📦 Position Size'},pos_val:{ar:'💵 قيمة الصفقة',en:'💵 Position Value'},leverage:{ar:'📊 الرافعة',en:'📊 Leverage'},exp_profit:{ar:'🎯 الربح المتوقع',en:'🎯 Expected Profit'},sl_loss:{ar:'🛑 خسارة SL',en:'🛑 SL Loss'},
  no_data:{ar:'لا بيانات',en:'No data'},empty_port:{ar:'فارغة',en:'Empty'},
  fr_normal:{ar:'طبيعي',en:'Normal'},fr_elevated:{ar:'مرتفع',en:'Elevated'},fr_danger:{ar:'⚠️ خطر',en:'⚠️ Danger'},fr_opportunity:{ar:'فرصة',en:'Opportunity'},
  fr_desc:{ar:'🔴 FR عالي = خطر | 🟢 FR سلبي = فرصة',en:'🔴 High FR = Risk | 🟢 Negative FR = Opportunity'},
  oi_desc:{ar:'📈 OI ↑ مع السعر = حركة حقيقية',en:'📈 OI ↑ with price = Real move'},
  ls_desc:{ar:'⚖️ نسبة Long/Short للمتداولين الكبار',en:'⚖️ Top trader Long/Short ratio'},
  cor_desc:{ar:'🔗 مصفوفة الارتباط (14 يوم)',en:'🔗 Correlation Matrix (14D)'},
  unlock_desc:{ar:'🔓 فتح التوكنات القادمة',en:'🔓 Upcoming Token Unlocks'},
  social_desc:{ar:'📱 الأكثر بحثاً ونقاشاً',en:'📱 Most searched & discussed'},
...
/* ═══════ LANG TOGGLE ═══════ */
function togLang(){
  lang=lang==='ar'?'en':'ar';localStorage.setItem('nxlang',lang);
  document.documentElement.lang=lang;document.documentElement.dir=lang==='ar'?'rtl':'ltr';
  document.body.dataset.lang=lang;
  document.getElementById('langB').textContent=lang==='ar'?'EN':'عربي';
  document.getElementById('sInp').placeholder=t('search_ph');
  document.getElementById('aAmt').placeholder=t('qty_ph');
  document.getElementById('aPr').placeholder=t('buy_price_ph');
  document.querySelectorAll('[data-t]').forEach(el=>{const k=el.dataset.t;if(TR[k])el.textContent=TR[k][lang]});
}
...
/* ═══════ THEME ═══════ */
function togTh(){const d=document.body.dataset.theme==='dark'?'light':'dark';document.body.dataset.theme=d;document.getElementById('thB').textContent=d==='dark'?'🌙':'☀️';if(tg){tg.setHeaderColor(d==='dark'?'#060b14':'#f7f9fc');tg.setBackgroundColor(d==='dark'?'#020408':'#f0f4f8')}localStorage.setItem('nxt7',d)}
(()=>{if(localStorage.getItem('nxt7')==='light')togTh();if(lang==='en')togLang()})();
...
async function loadFutures(){
  ...
  // Fallback: if LS empty, calculate from FR data
  if(!Object.keys(LS).length){for(const s of WL.slice(0,6)){const fr=FR[s];if(fr){const bias=fr.rate>0?55+Math.min(20,fr.rate*200):45-Math.min(15,Math.abs(fr.rate)*200);LS[s]={long:Math.round(bias),short:Math.round(100-bias),ratio:+(bias/(100-bias)).toFixed(2)}}}}
}
...
function frRow(s,d){const cls=d.rate>0.05?'dn':d.rate<-0.01?'up':'warn';const w=Math.min(48,Math.abs(d.rate)*500);const side=d.rate>=0?'left:50%':'right:50%';const lbl=d.rate>0.05?t('fr_danger'):d.rate>0.02?t('fr_elevated'):d.rate<-0.01?t('fr_opportunity'):t('fr_normal');
...
async function loadDash(){
  ...
  document.getElementById('dashBrk').innerHTML=brks.length?brks.map((r,i)=>coinRow(r.s,{p:r.p,c:r.c,v:r.v},i+1,`${t('prob')} ${Math.min(95,r.brk+40)}%`)).join(''):`<div class="muted">${t('no_brk')}</div>`;
...
async function runScan(){
  ...
  document.getElementById('scanI').textContent=`📊 ${Object.keys(T).length} ${lang==='ar'?'عملة':'coins'} | ✅ ${f.length} ${lang==='ar'?'نتيجة':'results'}`;
...
async function loadIntel(){
  ...
  document.getElementById('whT').textContent=fmt(whs.reduce((s,w)=>s+w.v*.05,0));
  ...
  document.getElementById('whAL').innerHTML=whs.map(r=>scanItem({...r,score:Math.min(100,r.acc+r.wh)})).join('')||`<div class="empty"><div class="empty-ic">🐋</div><div class="empty-tx">${t('no_whale')}</div></div>`;
}
async function loadFR(){if(!Object.keys(FR).length)await loadFutures();
  document.getElementById('frList').innerHTML=`<div class="muted">${t('fr_desc')}</div>`+Object.entries(FR).filter(([s])=>WL.includes(s)).sort((a,b)=>Math.abs(b[1].rate)-Math.abs(a[1].rate)).map(([s,d])=>frRow(s,d)).join('');
}
async function loadOIPage(){if(!Object.keys(OI).length)await loadFutures();
  document.getElementById('oiList').innerHTML=`<div class="muted">${t('oi_desc')}</div>`+Object.entries(OI).sort((a,b)=>b[1]-a[1]).map(([s,v])=>{const d=T[s];return`<div class="fr-row"><span class="fr-sym">${s}</span><span style="font-family:var(--fm);font-size:11px;color:var(--neon);font-weight:700">${fmt(v)}</span><span class="cr-ch ${d&&d.c>=0?'up':'dn'}">${d?(d.c>=0?'+':'')+d.c.toFixed(1)+'%':'--'}</span></div>`}).join('');
}
async function loadLSPage(){if(!Object.keys(LS).length)await loadFutures();
  document.getElementById('lsList').innerHTML=`<div class="muted">${t('ls_desc')}</div>`+Object.entries(LS).map(([s,d])=>`<div style="background:var(--glass);border:1px solid var(--bdr);border-radius:10px;padding:10px;margin-bottom:5px"><div class="ls-head"><span class="ls-sym">${s}</span><span class="ls-ratio">${d.ratio.toFixed(2)}</span></div><div class="ls-bar"><div class="ls-l" style="width:${d.long}%"></div><div class="ls-s" style="width:${d.short}%"></div></div><div class="ls-nums"><span style="color:var(--up)">🟢 ${d.long.toFixed(1)}%</span><span style="color:var(--dn)">🔴 ${d.short.toFixed(1)}%</span></div></div>`).join('');
}
async function loadCorrelation(){
  ...
  let h=`<div class="muted">${t('cor_desc')}</div><div style="display:grid;grid-template-columns:auto repeat(${coins.length},1fr);gap:2px;font-size:8px;font-family:var(--fm)"><div></div>`;
  coins.forEach(s=>h+=`<div style="text-align:center;font-weight:700">${s}</div>`);
  coins.forEach(a=>{h+=`<div style="font-weight:700">${a}</div>`;coins.forEach(b=>{if(!prices[a]||!prices[b]){h+=`<div style="text-align:center">--</div>`;return}const c=a===b?1:corr(prices[a],prices[b]);const bg=c>.7?'var(--ud)':c<-.3?'var(--dd)':'transparent';const col=c>.5?'var(--up)':c<-.3?'var(--dn)':'var(--t2)';h+=`<div style="text-align:center;padding:3px;border-radius:3px;background:${bg};color:${col};font-weight:700">${c.toFixed(2)}</div>`})});
...
async function loadLiq(){if(!Object.keys(T).length)await loadTk();
  ...
  let h='';for(const s of['BTC','ETH','SOL','BNB','XRP']){const ob=await fj(BN+`/depth?symbol=${s}USDT&limit=10`);if(!ob)continue;const bids=ob.bids.map(b=>+b[0]*+b[1]),asks=ob.asks.map(a=>+a[0]*+a[1]),bT=bids.reduce((a,b)=>a+b,0),aT=asks.reduce((a,b)=>a+b,0),r=aT>0?bT/aT:1,mx=Math.max(...bids,...asks);let pr,pc;if(r>1.3){pr='BUY';pc='up'}else if(r<.7){pr='SELL';pc='dn'}else{pr='NEUTRAL';pc='warn'}
...
async function loadSig(){const el=document.getElementById('sigL');let h='';
  for(const s of WL.slice(0,12)){const d=T[s];if(!d)continue;const kl=await fj(BN+`/klines?symbol=${s}USDT&interval=1h&limit=26`);if(!kl||kl.length<20)continue;const cl=kl.map(k=>+k[4]);let g=0,l=0;for(let i=cl.length-14;i<cl.length;i++){const df=cl[i]-cl[i-1];if(df>0)g+=df;else l+=Math.abs(df)}const rsi=100-100/(1+g/Math.max(l,.01));
    let act,cls,sc;if(rsi<30){act=t('buy_strong');cls='sig-buy';sc=''}else if(rsi<40){act=t('buy');cls='sig-buy';sc=''}else if(rsi>75){act=t('sell');cls='sig-sell';sc='sell'}else{act=t('hold');cls='sig-hold';sc='hold'}
    const fr=FR[s];h+=`<div class="sig ${sc}"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px"><span style="font-weight:800;font-family:var(--fd);font-size:13px">${s} <span style="font-size:10px;color:var(--t2)">${fP(d.p)}</span></span><span class="sig-act ${cls}">${act}</span></div><div class="sig-inds"><span class="sig-ind">RSI:${rsi.toFixed(0)}</span>${fr?`<span class="sig-ind">FR:${fr.rate.toFixed(3)}%</span>`:''}</div></div>`}
  el.innerHTML=h||`<div class="muted">${t('scanning')}</div>`;
}
async function loadNews(){const d=await fj(CG+'/search/trending');document.getElementById('newsL').innerHTML=d?.coins?d.coins.slice(0,10).map(c=>`<div class="cr" style="margin-bottom:4px"><div class="cr-l"><div class="cr-ic" style="background:var(--ud);color:var(--up);border:1px solid rgba(0,255,136,.1)">🔥</div><div><div class="cr-n">${c.item.name}</div><div class="cr-sub">${c.item.symbol} • Rank #${c.item.market_cap_rank||'N/A'}</div></div></div><div class="cr-r"><span class="cr-ch up">Trending</span></div></div>`).join(''):`<div class="muted">${t('no_data')}</div>`}
async function loadUnlocks(){const unlocks=[{sym:'ARB',date:'2026-04-16',amount:'$92M',pct:'3.5%'},{sym:'OP',date:'2026-04-30',amount:'$78M',pct:'2.8%'},{sym:'SUI',date:'2026-05-01',amount:'$120M',pct:'4.2%'},{sym:'TIA',date:'2026-04-20',amount:'$85M',pct:'5.1%'},{sym:'APT',date:'2026-05-15',amount:'$65M',pct:'2.1%'},{sym:'SEI',date:'2026-04-10',amount:'$45M',pct:'3.8%'}];
  document.getElementById('unlockL').innerHTML=`<div class="muted">${t('unlock_desc')}</div>`+unlocks.map(u=>{const days=Math.ceil((new Date(u.date)-new Date)/864e5);const cls=days<7?'dn':days<30?'warn':'up';return`<div class="fr-row"><div><span class="fr-sym">${u.sym}</span><span style="font-size:9px;color:var(--t3);margin-${lang==='ar'?'right':'left'}:6px">${u.date}</span></div><div style="text-align:${lang==='ar'?'left':'right'}"><div style="font-family:var(--fm);font-size:11px;font-weight:700">${u.amount}</div><div style="font-size:9px;color:var(--${cls})">${days>0?days+' '+t('days'):t('today')} | ${u.pct}</div></div></div>`}).join('')}
async function loadSocial(){const d=await fj(CG+'/search/trending');document.getElementById('socialL').innerHTML=`<div class="muted">${t('social_desc')}</div>`+(d?.coins?d.coins.slice(0,10).map((c,i)=>`<div class="fr-row"><span style="font-size:10px;color:var(--t3)">${i+1}</span><span class="fr-sym">${c.item.symbol}</span><span style="font-size:10px;color:var(--t2)">${c.item.name}</span><span class="cr-ch up">🔥</span></div>`).join(''):`<div class="muted">${t('no_data')}</div>`)}
...
async function openCoin(sym){curCoin=sym;curTF='1h';document.getElementById('sRes').classList.remove('show');document.getElementById('sInp').value='';
  const d=T[sym]||{p:0,c:0,v:0,h:0,l:0};document.getElementById('cmT').textContent=sym+'/USDT';document.getElementById('cmP').textContent=fP(d.p);
  document.getElementById('cmC').style.color=d.c>=0?'var(--up)':'var(--dn)';document.getElementById('cmC').textContent=(d.c>=0?'+':'')+d.c.toFixed(2)+'%';
  document.getElementById('cmSts').innerHTML=`<div class="st"><div class="st-l">VOL</div><div class="st-v" style="color:var(--neon)">${fmt(d.v)}</div></div><div class="st"><div class="st-l">HIGH</div><div class="st-v" style="color:var(--up)">${fP(d.h)}</div></div><div class="st"><div class="st-l">LOW</div><div class="st-v" style="color:var(--dn)">${fP(d.l)}</div></div>`;
  ...
  if(d.by)ex+=`<div class="fr-row"><span>🔄 Bybit</span><span class="fr-val">${fP(d.by)} (Δ${((d.by-d.p)/d.p*100).toFixed(2)}%)</span></div>`;
  document.getElementById('cmExtra').innerHTML=ex;
  openMo('coinMo');document.querySelectorAll('.chart-tf').forEach(b=>{b.classList.remove('act');if(b.dataset.t2==='1h')b.classList.add('act')});await drawChart(sym,'1h')}
...
async function drawChart(sym,tf){
  ...
  const kl=await fj(BN+`/klines?symbol=${sym}USDT&interval=${tf}&limit=80`);if(!kl||!kl.length){ctx.fillStyle='#4a5568';ctx.font='11px Syne';ctx.textAlign='center';ctx.fillText(t('no_data'),W/2,H/2);return}
...
/* ═══════ PORTFOLIO ═══════ */
const sP=()=>localStorage.setItem('nxp7',JSON.stringify(portfolio));
...
function renderPort(){let tV=0,tC=0;portfolio.forEach(p=>{const d=T[p.sym];if(d){tV+=d.p*p.amt;tC+=p.bp*p.amt}});const pnl=tC>0?((tV-tC)/tC*100):0;document.getElementById('pVal').textContent=tV>0?fmt(tV):'$0';const pE=document.getElementById('pCh');if(tC>0){pE.textContent=`${pnl>=0?'+':''}${pnl.toFixed(2)}% (${fmt(tV-tC)})`;pE.style.color=pnl>=0?'var(--up)':'var(--dn)'}else{pE.textContent=t('add_coins');pE.style.color='var(--t3)'}
  document.getElementById('pList').innerHTML=portfolio.length?portfolio.map((p,i)=>{const d=T[p.sym],cp=d?d.p:0,v=cp*p.amt,pnl=p.bp>0?((cp-p.bp)/p.bp*100):0;const bg=COL[p.sym]||'#444';return`<div class="port-i"><div style="display:flex;align-items:center;gap:8px"><div class="cr-ic" style="background:${bg}0a;color:${bg};border:1px solid ${bg}22;width:26px;height:26px;font-size:9px">${p.sym.slice(0,2)}</div><div><div class="cr-n">${p.sym}</div><div class="cr-sub">${p.amt} × ${fP(cp)}</div></div></div><div style="text-align:${lang==='ar'?'left':'right'}"><div class="cr-p">${fmt(v)}</div><div style="font-family:var(--fm);font-size:9px;font-weight:700;color:${pnl>=0?'var(--up)':'var(--dn)'}">${p.bp>0?`${pnl>=0?'+':''}${pnl.toFixed(1)}%`:'--'}</div><div style="font-size:7px;color:var(--t3);cursor:pointer" onclick="rmPort(${i})">🗑</div></div></div>`}).join(''):`<div class="empty"><div class="empty-ic">💼</div><div class="empty-tx">${t('empty_port')}</div></div>`}
...
function calcRisk(){const cap=+document.getElementById('rcCap').value,risk=+document.getElementById('rcRisk').value,entry=+document.getElementById('rcEntry').value,sl=+document.getElementById('rcSL').value,tp=+document.getElementById('rcTP').value;
  if(!cap||!entry||!sl){document.getElementById('rcRes').innerHTML=`<div style="text-align:center;color:var(--t3);padding:12px;font-size:11px">${t('enter_data')}</div>`;return}
  ...
  document.getElementById('rcRes').innerHTML=`<div class="rc-row"><span>${t('risk_amt')}</span><span class="rc-val" style="color:var(--dn)">${fmt(rAmt)}</span></div><div class="rc-row"><span>${t('pos_size')}</span><span class="rc-val">${pos.toFixed(4)}</span></div><div class="rc-row"><span>${t('pos_val')}</span><span class="rc-val">${fmt(posV)}</span></div><div class="rc-row"><span>${t('leverage')}</span><span class="rc-val" style="color:${lev>10?'var(--dn)':lev>5?'var(--warn)':'var(--up)'}">${lev.toFixed(1)}x</span></div>${tp?`<div class="rc-row"><span>${t('exp_profit')}</span><span class="rc-val" style="color:var(--up)">${fmt(rew)}</span></div><div class="rc-row"><span>⚖️ R/R</span><span class="rc-val" style="color:${rr>=2?'var(--up)':rr>=1?'var(--warn)':'var(--dn)'}">1:${rr.toFixed(1)}</span></div>`:''}<div class="rc-row"><span>${t('sl_loss')}</span><span class="rc-val" style="color:var(--dn)">-${fmt(rAmt)}</span></div>`}

/* ═══════ AUTO REFRESH ═══════ */
let refreshTimer=null;
async function autoRefresh(){
  try{
    await loadTk();await loadFutures();
    // Update pulse bar
    const brkCount=Object.values(T).filter(t=>t.c>=8).length;
    document.getElementById('pBrk').textContent=brkCount;
    document.getElementById('brkC').textContent=brkCount;
    const btcFR=FR.BTC;if(btcFR)document.getElementById('pFR').textContent=(btcFR.rate>=0?'+':'')+btcFR.rate.toFixed(4)+'%';
...
async function init(){
  ...
  // Full dashboard refresh every 3 minutes
  setInterval(async()=>{
