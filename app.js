...
/* SOUND NOTIFICATIONS */
function playSound(type){try{var el=document.getElementById(type==='ultra'?'sndUltra':'sndWhale');if(el){el.currentTime=0;el.play().catch(function(){})}}catch(e){}}
...
function indTab(i,btn){document.getElementById('pg-ind').querySelectorAll('.tab').forEach(function(b){b.classList.remove('act')});btn.classList.add('act');['ind0','ind1','ind2'].forEach(function(id,j){document.getElementById(id).style.display=j===i?'block':'none'});if(i===0)loadFR();if(i===1)loadOI();if(i===2)loadCor()}
