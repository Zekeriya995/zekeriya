...
const fj=async u=>{try{var r=await fetch(u);if(!r.ok)throw 0;return r.json()}catch(e){return null}};
...
function calcMACD(c){if(c.length<26)return{h:0};var ema=function(d,p){var k=2/(p+1),e=d[0];for(var i=1;i<d.length;i++)e=d[i]*k+e*(1-k);return e};return{h:ema(c.slice(-12),12)-ema(c,26)}}
...
/* SOUND NOTIFICATIONS — Web Audio API (loud!) */
function playSound(type){try{var ac=new(window.AudioContext||window.webkitAudioContext)();var osc=ac.createOscillator();var gain=ac.createGain();osc.connect(gain);gain.connect(ac.destination);gain.gain.value=0.3;
  if(type==='ultra'){osc.frequency.value=880;osc.type='sine';gain.gain.value=0.4;osc.start();osc.stop(ac.currentTime+0.15);setTimeout(function(){var o2=ac.createOscillator();var g2=ac.createGain();o2.connect(g2);g2.connect(ac.destination);g2.gain.value=0.4;o2.frequency.value=1100;o2.type='sine';o2.start();o2.stop(ac.currentTime+0.2)},200)}
  else{osc.frequency.value=660;osc.type='triangle';osc.start();osc.stop(ac.currentTime+0.2)}}catch(e){try{document.getElementById('sndAlert').play()}catch(e2){}}}
...
function notify(sym,type,score){var k=sym+'_'+type+'_'+new Date().getHours();if(notifiedSet[k])return;notifiedSet[k]=true;localStorage.setItem('nxnot10',JSON.stringify(notifiedSet));playSound(type);
  if(type==='ultra')showPopup('⭐',sym+' — ULTRA Signal!','Score: '+score+' | '+(lang==='ar'?'ادخل الآن!':'Enter now!'));
  else if(type==='whale')showPopup('🐋',sym+' — '+(lang==='ar'?'تجميع حيتان!':'Whale detected!'),(lang==='ar'?'نشاط غير عادي':'Unusual activity'));
  else if(type==='gem')showPopup('💎',sym+' — '+(lang==='ar'?'جوهرة مكتشفة!':'Gem found!'),(lang==='ar'?'عملة صغيرة بحركة قوية':'Small cap with strong move'))}
/* LANG/THEME/NAV */
function togLang(){lang=lang==='ar'?'en':'ar';localStorage.setItem('nxlang',lang);document.documentElement.lang=lang;document.documentElement.dir=lang==='ar'?'rtl':'ltr';document.body.dataset.lang=lang;document.getElementById('langB').textContent=lang==='ar'?'EN':'عربي';document.getElementById('sInp').placeholder=t('search_ph');document.querySelectorAll('[data-t]').forEach(function(el){var k=el.dataset.t;if(TR[k])el.textContent=TR[k][lang]})}
function togTh(){var d=document.body.dataset.theme==='dark'?'light':'dark';document.body.dataset.theme=d;document.getElementById('thB').textContent=d==='dark'?'🌙':'☀️';if(tg){tg.setHeaderColor(d==='dark'?'#060b14':'#f7f9fc');tg.setBackgroundColor(d==='dark'?'#020408':'#f0f4f8')}localStorage.setItem('nxt10',d)}
(function(){if(localStorage.getItem('nxt10')==='light')togTh();if(lang==='en')togLang()})();
...
function pTab(i,btn){document.getElementById('pg-me').querySelectorAll('.tab').forEach(function(b){b.classList.remove('act')});btn.classList.add('act');['p0','p1','p2'].forEach(function(id,j){document.getElementById(id).style.display=j===i?'block':'none'})}
...
/* WS */
function initWS(){if(ws)ws.close();ws=new WebSocket('wss://stream.binance.com:9443/stream?streams='+WL.map(function(s){return s.toLowerCase()+'usdt@miniTicker'}).join('/'));ws.onmessage=function(e){var d=JSON.parse(e.data).data;if(!d)return;var s=d.s.replace('USDT','');T[s]=Object.assign(T[s]||{},{p:+d.c,c:+d.P,v:+d.q,h:+d.h,l:+d.l,src:'BN'})};ws.onclose=function(){setTimeout(initWS,3000)};ws.onerror=function(){ws.close()}}
/* LOAD TICKERS — ALL 3 EXCHANGES */
async function loadTk(){
  var bn=await fj(BN+'/ticker/24hr');if(bn)bn.filter(function(x){return x.symbol.endsWith('USDT')&&+x.quoteVolume>100000}).forEach(function(x){var s=x.symbol.replace('USDT','');T[s]={p:+x.lastPrice,c:+x.priceChangePercent,v:+x.quoteVolume,h:+x.highPrice,l:+x.lowPrice,src:'BN'}});
  ...
  try{var cbR=await fj(CB+'/exchange-rates?currency=USD');if(cbR&&cbR.data&&cbR.data.rates){var rates=cbR.data.rates;Object.keys(rates).forEach(function(c){var r=+rates[c];if(r>0)CBP[c]=1/r})}}catch(e){}
  var el=document.getElementById('tkrEl');var items=WL.filter(function(s){return T[s]}).slice(0,16);var h='';for(var r=0;r<2;r++)items.forEach(function(s){var d=T[s],up=d.c>=0;h+='<div class="tkr-i"><span class="tkr-sym">'+s+'</span><span style="font-family:var(--fm);font-size:9px;color:var(--t2)">'+fP(d.p)+'</span><div class="spark">'+(up?[4,6,5,8,7,10,12,14]:[14,12,10,8,6,5,4,3]).map(function(v){return'<b style="height:'+v+'px;background:var(--'+(up?'up':'dn')+')"></b>'}).join('')+'</div><span class="tkr-c '+(up?'up':'dn')+'">'+(up?'+':'')+d.c.toFixed(1)+'%</span></div>'});el.innerHTML=h}
async function loadFutures(){
  var fd=await fj(BF+'/premiumIndex');if(fd)fd.forEach(function(d){var s=d.symbol.replace('USDT','');FR[s]={rate:+d.lastFundingRate*100,mark:+d.markPrice}});if(FR.BTC)document.getElementById('pFR').textContent=(FR.BTC.rate>=0?'+':'')+FR.BTC.rate.toFixed(4)+'%';
...
async function deepAnalyze(cands){var results=[];var top=cands.slice(0,30);
  ...
  for(var ci=0;ci<top.length;ci++){var c=top[ci];var ds=c.score,dt=c.tags.slice();
    ...
    var kl=klData[c.s];if(kl&&kl.length>=20){var vols=kl.map(function(k){return+k[5]});var closes=kl.map(function(k){return+k[4]});
      ...
      var macd=calcMACD(closes);if(macd.h>0){ds+=12;checks.macd=true;dt.push('MACD✅')}
    }
    ...
    /* CHECK 5: Funding Rate 15% */
    if(FR[c.s]){if(FR[c.s].rate<0.01){ds+=10;checks.fr=true;dt.push('FR✅')}
      else if(FR[c.s].rate<-0.02){ds+=15;checks.fr=true;dt.push('FR⬇️🟢')}}
    ...
    if(isUltra){recSig(c.s,'ultra');notify(c.s,'ultra',ds)}
    if(c.tags.some(function(t){return t.includes('ACC')||t.includes('STEALTH')||t.includes('EARLY')})){recSig(c.s,'whale');if(checks.ob)notify(c.s,'whale',ds)}
...
function renderAcc(id){var a=getAcc();var el=document.getElementById(id);if(!el)return;el.innerHTML='<div class="acc-card"><div style="text-align:center"><div style="font-family:var(--fd);font-size:36px;font-weight:800;color:'+(a.rate>=70?'var(--up)':a.rate>=50?'var(--warn)':'var(--t2)')+'">'+a.rate+'%</div><div style="font-size:11px;color:var(--t2)">'+a.hits+'/'+a.total+' '+t('confirmed')+'</div></div></div>'}
/* 💰 STABLECOIN FLOW INDICATOR */
async function loadStableFlow(){
  try{
    /* Fetch USDT and USDC klines (7 days) to compare volumes */
    var usdtKl=await fj(BN+'/klines?symbol=USDTDAI&interval=1d&limit=7');
    var usdtTk=T['USDT']||await fj(BN+'/ticker/24hr?symbol=USDTDAI');
    /* Get USDT volume from all USDT pairs */
    var usdtVol=0,usdcVol=0;
    Object.entries(T).forEach(function(e){var s=e[0],d=e[1];if(d.src==='BN'){usdtVol+=d.v}});
    /* Get USDC pairs volume */
    var usdcPairs=await fj(BN+'/ticker/24hr');
    if(usdcPairs)usdcPairs.filter(function(x){return x.symbol.endsWith('USDC')}).forEach(function(x){usdcVol+= +x.quoteVolume});
    /* Calculate total stablecoin volume today */
    var totalStableVol=usdtVol+usdcVol;
    /* Get BTC volume as reference for ratio */
    var btcVol=T['BTC']?T['BTC'].v:1;
    /* Stablecoin Dominance Ratio: high = people selling crypto for stables */
    /* We use ratio of stablecoin trading vs total to gauge sentiment */
    var stableRatio=totalStableVol>0?Math.min(100,Math.round((usdcVol/(usdcVol+btcVol*10))*100)):50;
    /* Invert: high USDC buying = people converting crypto to stable = bearish */
    var flowIndex=stableRatio;
    /* Also check BTC price trend as confirmation */
    var btcChange=T['BTC']?T['BTC'].c:0;
    if(btcChange<-3)flowIndex=Math.min(100,flowIndex+15);
    if(btcChange>3)flowIndex=Math.max(0,flowIndex-15);
    /* Check Fear & Greed as additional signal */
    if(fgValue<25)flowIndex=Math.min(100,flowIndex+10);
    if(fgValue>75)flowIndex=Math.max(0,flowIndex-10);
    /* Clamp */
    flowIndex=Math.max(5,Math.min(95,flowIndex));
    /* Update UI */
...
/* RENDER */
function coinRow(s,d,i,sub){var up=d.c>=0;var bg=COL[s]||'#444';return'<div class="cr" onclick="openCoin(\''+s+'\')"><div class="cr-l">'+(i!==undefined?'<div class="cr-rk">'+i+'</div>':'')+'<div class="cr-ic" style="background:'+bg+'0a;color:'+bg+';border:1px solid '+bg+'22">'+s.slice(0,2)+'</div><div><div class="cr-n">'+s+'</div><div class="cr-sub">'+(sub||fmt(d.v))+'</div></div></div><div class="cr-spark">'+(up?[4,6,5,8,12,16,19,22]:[22,18,14,10,8,6,5,4]).map(function(v){return'<b style="height:'+v+'px;background:var(--'+(up?'up':'dn')+')"></b>'}).join('')+'</div><div class="cr-r"><div class="cr-p">'+fP(d.p)+'</div><div class="cr-ch '+(up?'up':'dn')+'">'+(up?'+':'')+d.c.toFixed(1)+'%</div></div></div>'}
function ultraCard(r){savePred(r.s,r.p,r.p*1.1,r.score);var src=[];if(T[r.s])src.push('Binance');if(r.by)src.push('Bybit');if(CBP[r.s])src.push('Coinbase');return'<div class="ultra" onclick="openCoin(\''+r.s+'\')"><div class="u-badge">⭐ '+(r.ultra?'🟢 CONFIRMED':'🟡 PROBABLE')+' — '+r.passed+'/'+r.total+' CHECKS</div><div style="display:flex;justify-content:space-between;align-items:flex-start"><div><div class="u-sym">'+r.s+'/USDT</div><div class="u-price"><span style="color:var(--up);font-weight:700">+'+r.c.toFixed(1)+'%</span> '+fP(r.p)+'</div></div><div style="text-align:center"><div class="u-score-val">'+r.score+'</div><div class="u-score-lbl">SCORE</div></div></div><div style="margin:8px 0">'+timeBadge(r.detectedAt)+'</div><div class="u-conf">'+Object.entries(r.checks).map(function(e){return'<div class="u-conf-i '+(e[1]?'pass':'fail')+'">'+e[0]+' '+(e[1]?'✅':'❌')+'</div>'}).join('')+'</div><div class="u-tags">'+r.tags.slice(0,6).map(function(x){return'<span class="u-tag" style="background:var(--ud);color:var(--up)">'+x+'</span>'}).join('')+'</div><div class="src-row">'+src.map(function(s){return'<span class="src-badge">'+s+'</span>'}).join('')+'</div><div class="u-range" style="margin-top:8px"><div style="font-size:10px;font-weight:700;margin-bottom:4px">🎯 Target</div><div class="u-range-row"><span style="color:var(--up)">Conservative</span><span style="font-weight:700">'+fP(r.p*1.08)+'</span></div><div class="u-range-row"><span style="color:var(--neon)">Target</span><span style="font-weight:700">'+fP(r.p*1.15)+'</span></div><div class="u-range-row"><span style="color:var(--dn)">🛑 Stop</span><span style="font-weight:700;color:var(--dn)">'+fP(r.p*0.93)+'</span></div></div></div>'}
function whaleCard(r){var wt=getSigTime(r.s,'whale');var amt=r.v*0.05;var bp=r.p*(1-Math.abs(r.c)/200);var str=amt>5e6?{t:lang==='ar'?'قوي جداً':'Very Strong',c:'str-strong'}:amt>1e6?{t:lang==='ar'?'عادي':'Normal',c:'str-normal'}:{t:lang==='ar'?'ضعيف':'Weak',c:'str-weak'};var src=[];if(T[r.s])src.push('Binance');if(r.by)src.push('Bybit');if(CBP[r.s])src.push('Coinbase');return'<div class="whale-card" onclick="openCoin(\''+r.s+'\')"><div class="whale-head"><div class="whale-sym">🐋 '+r.s+'/USDT <span class="str-badge '+str.c+'">'+str.t+'</span></div>'+timeBadge(wt)+'</div><div class="whale-grid"><div class="whale-item"><div class="whale-item-v" style="color:var(--neon)">'+fmt(amt)+'</div><div class="whale-item-l">'+(lang==='ar'?'كمية الشراء':'Buy Amount')+'</div></div><div class="whale-item"><div class="whale-item-v">'+fP(bp)+'</div><div class="whale-item-l">'+(lang==='ar'?'سعر الشراء':'Buy Price')+'</div></div><div class="whale-item"><div class="whale-item-v" style="color:var(--up)">'+fP(r.p)+'</div><div class="whale-item-l">'+(lang==='ar'?'الحالي':'Current')+'</div></div><div class="whale-item"><div class="whale-item-v" style="color:'+(r.c>=0?'var(--up)':'var(--dn)')+'">'+(r.c>=0?'+':'')+r.c.toFixed(1)+'%</div><div class="whale-item-l">24H</div></div></div><div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px"><div class="src-row">'+src.map(function(s){return'<span class="src-badge">'+s+'</span>'}).join('')+'</div><span style="font-family:var(--fm);font-size:9px;color:var(--t2)">Vol:'+fmt(r.v)+'</span></div></div>'}
...
async function loadDash(){
  ...
  var cands=quickScan();var results=await deepAnalyze(cands);cache.scan=results;cache.scanTime=Date.now();
  ...
  /* L/S on homepage */
  var lsC=['BTC','ETH','SOL'].filter(function(s){return LS[s]});document.getElementById('dashLS').innerHTML=lsC.map(function(s){var d=LS[s];return'<div class="ls-item"><div class="ls-head"><span class="ls-sym">'+s+'</span><span class="ls-ratio"'+(d.ratio<0.8||d.ratio>1.8?' style="color:var(--dn)"':'')+'>'+d.ratio.toFixed(2)+'</span></div><div class="ls-bar"><div class="ls-l" style="width:'+d.long+'%"></div><div class="ls-s" style="width:'+d.short+'%"></div></div><div class="ls-nums"><span style="color:var(--up)">🟢 '+d.long.toFixed(0)+'%</span><span style="color:var(--dn)">🔴 '+d.short.toFixed(0)+'%</span></div></div>'}).join('')||'<div class="muted">'+t('scanning')+'</div>';
  renderAcc('accCard');
}
...
function renderScanResults(results){var mode=(document.querySelector('#fltM .flt-b.act')||{dataset:{m:'all'}}).dataset.m;var f=results;if(mode==='ultra')f=results.filter(function(r){return r.ultra||r.confirmed});else if(mode==='brk')f=results.filter(function(r){return r.c>=1&&r.score>=35});else if(mode==='fr')f=results.filter(function(r){return r.fr!=null}).sort(function(a,b){return Math.abs(b.fr||0)-Math.abs(a.fr||0)});document.getElementById('scanI').textContent='📊 '+Object.keys(T).length+' '+(lang==='ar'?'عملة':'coins')+' → ✅ '+f.length;document.getElementById('scanR').innerHTML=f.length?f.slice(0,30).map(scanItem).join(''):'<div class="empty"><div class="empty-ic">📡</div><div class="empty-tx">'+t('no_data')+'</div></div>'}
/* WHALE PAGE */
async function loadWhales(){if(cache.scan&&Date.now()-cache.scanTime<CACHE_TTL){renderWhaleResults(cache.scan);return}var c=quickScan();var r=await deepAnalyze(c);cache.scan=r;cache.scanTime=Date.now();renderWhaleResults(r)}
function renderWhaleResults(results){var w=results.filter(function(x){return x.tags.some(function(t){return t.includes('ACC')||t.includes('STEALTH')||t.includes('EARLY')||t.includes('BOTTOM')})||(x.v>5e7&&Math.abs(x.c)<3)||(x.checks.ob&&x.v>1e7)}).slice(0,15);document.getElementById('whT').textContent=fmt(w.reduce(function(s,x){return s+x.v*.05},0));document.getElementById('whB').textContent=fmt(w.filter(function(x){return x.c>0}).reduce(function(s,x){return s+x.v*.05},0));document.getElementById('whS').textContent=fmt(w.filter(function(x){return x.c<0}).reduce(function(s,x){return s+x.v*.05},0));document.getElementById('whAL').innerHTML=w.length?w.map(whaleCard).join(''):'<div class="empty"><div class="empty-ic">🐋</div><div class="empty-tx">'+t('no_whale')+'</div></div>';renderAcc('whAccCard')}
/* INDICATORS PAGE */
...
async function drawChart(sym,tf){var cv=document.getElementById('chCv'),ctx=cv.getContext('2d');var dpr=window.devicePixelRatio||1;cv.width=cv.clientWidth*dpr;cv.height=280*dpr;ctx.scale(dpr,dpr);var W=cv.clientWidth,H=280;ctx.clearRect(0,0,W,H);var kl=await fj(BN+'/klines?symbol='+sym+'USDT&interval='+tf+'&limit=80');if(!kl||!kl.length){ctx.fillStyle='#4a5568';ctx.font='11px Syne';ctx.textAlign='center';ctx.fillText(t('no_data'),W/2,H/2);return}
  var data=kl.map(function(k){return{o:+k[1],h:+k[2],l:+k[3],c:+k[4],v:+k[5]}});
  var mH=inds.rsi?H*.62:H-30;var priceW=50;var cw=(W-priceW-4)/data.length,ch=mH-30;
  var maxP=Math.max.apply(null,data.map(function(d){return d.h}));
  var minP=Math.min.apply(null,data.map(function(d){return d.l}));
  var range=maxP-minP;if(range===0)range=maxP*0.01;
  maxP+=range*0.02;minP-=range*0.02;range=maxP-minP;
  var cs=getComputedStyle(document.body);
  var upC='#00ff88',dnC='#ff3860',upCa='rgba(0,255,136,',dnCa='rgba(255,56,96,';
  var yS=function(p){return 12+ch-((p-minP)/range)*ch};
  /* Grid + Price labels on RIGHT */
  ctx.textAlign='right';
  for(var i=0;i<=5;i++){var y=12+ch/5*i;var price=maxP-range/5*i;
    ctx.strokeStyle='rgba(56,72,96,.15)';ctx.lineWidth=.5;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W-priceW,y);ctx.stroke();
    ctx.fillStyle='#6e82a0';ctx.font='8px Geist Mono';ctx.fillText(fP(price),W-4,y+3)}
  /* Volume bars with gradient */
  if(inds.vol){var mV=Math.max.apply(null,data.map(function(d){return d.v}));
    data.forEach(function(d,i){var up=d.c>=d.o;var vH=Math.max(2,d.v/mV*30);var x=2+i*cw;
      var grd=ctx.createLinearGradient(0,mH-vH,0,mH);
      grd.addColorStop(0,up?upCa+'.15)':dnCa+'.15)');grd.addColorStop(1,up?upCa+'.02)':dnCa+'.02)');
      ctx.fillStyle=grd;ctx.fillRect(x+1,mH-vH,cw-2,vH)})}
  /* EMA line */
  if(inds.sma&&data.length>=21){ctx.beginPath();ctx.strokeStyle='rgba(91,156,255,.6)';ctx.lineWidth=1.5;
    for(var i=20;i<data.length;i++){var avg=data.slice(i-20,i+1).reduce(function(s,d){return s+d.c},0)/21;var x=2+i*cw+cw/2;
      if(i===20)ctx.moveTo(x,yS(avg));else ctx.lineTo(x,yS(avg))};ctx.stroke()}
  /* S/R levels */
  if(inds.sr){var lows=data.map(function(d){return d.l}),highs=data.map(function(d){return d.h});
    for(var i=2;i<data.length-2;i++){
      if(lows[i]<lows[i-1]&&lows[i]<lows[i-2]&&lows[i]<lows[i+1]&&lows[i]<lows[i+2]){ctx.strokeStyle='rgba(0,255,136,.25)';ctx.setLineDash([3,3]);ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(0,yS(lows[i]));ctx.lineTo(W-priceW,yS(lows[i]));ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='rgba(0,255,136,.4)';ctx.font='7px Geist Mono';ctx.textAlign='left';ctx.fillText('S',4,yS(lows[i])-3);ctx.textAlign='right'}
      if(highs[i]>highs[i-1]&&highs[i]>highs[i-2]&&highs[i]>highs[i+1]&&highs[i]>highs[i+2]){ctx.strokeStyle='rgba(255,56,96,.25)';ctx.setLineDash([3,3]);ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(0,yS(highs[i]));ctx.lineTo(W-priceW,yS(highs[i]));ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='rgba(255,56,96,.4)';ctx.font='7px Geist Mono';ctx.textAlign='left';ctx.fillText('R',4,yS(highs[i])-3);ctx.textAlign='right'}}}
  /* CANDLESTICKS — professional with glow */
  data.forEach(function(d,i){
    var x=2+i*cw+cw/2,bw=Math.max(2,cw*.7),up=d.c>=d.o;
    var col=up?upC:dnC;var colA=up?upCa:dnCa;
    /* Wick — thicker for visibility */
    ctx.strokeStyle=col;ctx.lineWidth=1.2;ctx.beginPath();ctx.moveTo(x,yS(d.h));ctx.lineTo(x,yS(d.l));ctx.stroke();
    /* Body with subtle glow */
    var top=yS(Math.max(d.o,d.c)),bot=yS(Math.min(d.o,d.c));var bodyH=Math.max(1.5,bot-top);
  ...
  /* Price label box */
  ctx.fillStyle=lastUp?upC:dnC;var lbW=48,lbH=14;
  ctx.fillRect(W-priceW,cpY-lbH/2,lbW,lbH);ctx.fillStyle='#000';ctx.font='bold 8px Geist Mono';ctx.textAlign='center';ctx.fillText(fP(lastP),W-priceW+lbW/2,cpY+3);ctx.textAlign='right';
  /* RSI Panel */
  if(inds.rsi&&data.length>=14){var rsiY=mH+10,rsiH=H*.18;var closes=data.map(function(d){return d.c}),rsis=[];
    for(var i=14;i<closes.length;i++){var g=0,l=0;for(var j=i-13;j<=i;j++){var df=closes[j]-closes[j-1];if(df>0)g+=df;else l+=Math.abs(df)};rsis.push(100-100/(1+g/Math.max(l,.001)))};
    /* RSI background */
    ctx.fillStyle='rgba(10,16,28,.5)';ctx.fillRect(0,rsiY-2,W-priceW,rsiH+4);
    /* RSI 70/30 zones */
    ctx.fillStyle='rgba(255,56,96,.04)';ctx.fillRect(0,rsiY,W-priceW,rsiH*0.3);
    ctx.fillStyle='rgba(0,255,136,.04)';ctx.fillRect(0,rsiY+rsiH*0.7,W-priceW,rsiH*0.3);
    /* RSI lines */
    ctx.strokeStyle='rgba(255,56,96,.15)';ctx.setLineDash([3,3]);ctx.lineWidth=.5;
    ctx.beginPath();ctx.moveTo(0,rsiY+rsiH*0.3);ctx.lineTo(W-priceW,rsiY+rsiH*0.3);ctx.stroke();
    ctx.beginPath();ctx.moveTo(0,rsiY+rsiH*0.7);ctx.lineTo(W-priceW,rsiY+rsiH*0.7);ctx.stroke();ctx.setLineDash([]);
    /* RSI curve */
    ctx.beginPath();ctx.strokeStyle='rgba(176,124,255,.8)';ctx.lineWidth=1.5;var off=data.length-rsis.length;
    rsis.forEach(function(v,i){var x=2+(i+off)*cw+cw/2,y=rsiY+rsiH-(v/100)*rsiH;if(i===0)ctx.moveTo(x,y);else ctx.lineTo(x,y)});ctx.stroke();
    /* RSI labels */
    ctx.fillStyle='#6e82a0';ctx.font='7px Geist Mono';ctx.textAlign='right';
    ctx.fillText('70',W-4,rsiY+rsiH*0.3+3);ctx.fillText('30',W-4,rsiY+rsiH*0.7+3);
    ctx.fillText('RSI:'+rsis[rsis.length-1].toFixed(0),W-4,rsiY+10)}}
/* HEATMAP */
...
async function loadGems(){if(!Object.keys(T).length)await loadTk();var gems=Object.entries(T).filter(function(e){var d=e[1];return d.v>500000&&d.v<5e7&&d.c>=5&&d.p<1}).sort(function(a,b){return b[1].c-a[1].c}).slice(0,20);
  document.getElementById('gemL').innerHTML=gems.length?gems.map(function(e,i){var s=e[0],d=e[1];recSig(s,'gem');var gt=getSigTime(s,'gem');
    return'<div class="whale-card" onclick="openCoin(\''+s+'\')"><div class="whale-head"><div class="whale-sym">💎 '+s+'/USDT <span class="str-badge '+(d.c>=15?'str-strong':d.c>=8?'str-normal':'str-weak')+'">'+(d.c>=15?(lang==='ar'?'🔥 ساخن':'🔥 Hot'):d.c>=8?(lang==='ar'?'⚡ نشط':'⚡ Active'):(lang==='ar'?'📈 صاعد':'📈 Rising'))+'</span></div>'+timeBadge(gt)+'</div><div class="whale-grid"><div class="whale-item"><div class="whale-item-v" style="color:var(--up)">'+fP(d.p)+'</div><div class="whale-item-l">'+(lang==='ar'?'السعر':'Price')+'</div></div><div class="whale-item"><div class="whale-item-v" style="color:var(--up)">+'+d.c.toFixed(1)+'%</div><div class="whale-item-l">24H</div></div><div class="whale-item"><div class="whale-item-v" style="color:var(--neon)">'+fmt(d.v)+'</div><div class="whale-item-l">Volume</div></div><div class="whale-item"><div class="whale-item-v" style="color:var(--warn)">'+fP(d.l)+'</div><div class="whale-item-l">Low</div></div></div></div>'}).join(''):'<div class="empty"><div class="empty-ic">💎</div><div class="empty-tx">'+(lang==='ar'?'لا جواهر حالياً':'No gems now')+'</div></div>';
  /* Notify for hot gems */
  gems.filter(function(e){return e[1].c>=15}).forEach(function(e){notify(e[0],'gem',0)})}
/* WATCHLIST */
...
/* INIT */
async function init(){document.getElementById('sInp').placeholder=t('search_ph');document.getElementById('notifB').dataset.c='0';await loadDash();renderPort();
  setInterval(async function(){try{await loadTk();await loadFutures()}catch(e){}},60000);
