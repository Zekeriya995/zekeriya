...
var curScanTab=0,curTradeFilter='all',curSmallFilter='all';
var SECTORS={ai:{ic:'🤖',n:{ar:'ذكاء اصطناعي',en:'AI'},coins:['FET','RNDR','TAO','NEAR','GRT','WLD','AKT','ARKM'],col:'#7c3aed'},gaming:{ic:'🎮',n:{ar:'ألعاب',en:'Gaming'},coins:['IMX','GALA','AXS','SAND','MANA','ENJ','PIXEL','BEAM'],col:'#06b6d4'},layer1:{ic:'⛓️',n:{ar:'Layer 1',en:'Layer 1'},coins:['ETH','SOL','AVAX','DOT','ATOM','NEAR','APT','SUI','SEI','ICP'],col:'#3b82f6'},layer2:{ic:'🔗',n:{ar:'Layer 2',en:'Layer 2'},coins:['ARB','OP','MATIC','IMX','MANTA','STRK'],col:'#8b5cf6'},defi:{ic:'💰',n:{ar:'DeFi',en:'DeFi'},coins:['UNI','AAVE','MKR','LDO','SNX','CRV','COMP','DYDX','GMX'],col:'#10b981'},meme:{ic:'🐕',n:{ar:'Meme',en:'Meme'},coins:['DOGE','PEPE','WIF','BONK','FLOKI','SHIB'],col:'#f59e0b'},rwa:{ic:'🏦',n:{ar:'RWA',en:'RWA'},coins:['ONDO','MKR','LINK','PENDLE'],col:'#64748b'},depin:{ic:'🌐',n:{ar:'DePIN',en:'DePIN'},coins:['FIL','AR','RNDR','HNT','THETA'],col:'#0ea5e9'},data:{ic:'⚡',n:{ar:'Data/Oracle',en:'Data/Oracle'},coins:['LINK','GRT','BAND','PYTH'],col:'#6366f1'}};
function getCoinSector(sym){for(var k in SECTORS)if(SECTORS[k].coins.includes(sym))return k;return null}
function scanTab(idx,btn){curScanTab=idx;document.querySelectorAll('#pg-scan>.big-tabs>.big-tab').forEach(function(b){b.classList.remove('act')});if(btn)btn.classList.add('act');['scanTrend','scanTrade','scanSmall'].forEach(function(id,j){var el=document.getElementById(id);if(el)el.style.display=j===idx?'block':'none'});if(idx===0)loadTrending();if(idx===1)loadTrading();if(idx===2)loadSmallCapsUI()}
...
function renderTrading(sigs){var f=sigs;if(curTradeFilter==='fast')f=sigs.filter(function(x){return x.type==='fast'});else if(curTradeFilter==='daily')f=sigs.filter(function(x){return x.type==='daily'});else if(curTradeFilter!=='all'){f=sigs.filter(function(x){return x.sec===curTradeFilter})}
  document.getElementById('scanI').textContent='📊 '+f.length+' '+(lang==='ar'?'إشارة':'signals');
  if(!f.length){document.getElementById('tradeList').innerHTML='<div class="empty"><div class="empty-ic">📡</div><div class="empty-tx">'+(lang==='ar'?'لا إشارات حالياً':'No signals now')+'</div></div>';return}
  var h='';f.slice(0,20).forEach(function(s,i){var tBg=s.type==='fast'?'var(--bd)':'var(--ud)';var tCol=s.type==='fast'?'var(--blue)':'var(--up)';var tLbl=s.type==='fast'?'⚡ Fast':'📊 Daily';var confCol=s.conf>=60?'var(--up)':s.conf>=40?'var(--warn)':'var(--t2)';var tb=getTierBadge(s.s);
    h+='<div class="whale-card" style="border-left:3px solid '+(s.ultra?'var(--ultra)':s.type==='fast'?'var(--blue)':'var(--up)')+';margin-bottom:8px">'
      +'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px"><div style="display:flex;align-items:center;gap:6px"><span style="font-size:11px;color:var(--t3);font-weight:800">#'+(i+1)+'</span><span style="font-weight:800;font-size:14px">'+(s.ultra?'⭐ ':s.confirmed?'🟢 ':'')+ s.s+(tb?' <span style="font-size:8px">'+tb+'</span>':'')+'</span>'
      +'<span style="font-size:8px;padding:2px 6px;border-radius:4px;background:'+tBg+';color:'+tCol+';font-weight:700">'+tLbl+'</span></div>'
      +'<span style="font-family:var(--fm);font-size:13px;font-weight:800;color:'+confCol+'">'+s.conf+'%</span></div>'
      +'<div style="display:flex;justify-content:space-between;font-family:var(--fm);font-size:10px;margin-bottom:6px"><span>💰 '+fP(s.p)+'</span><span style="color:'+(s.c>=0?'var(--up)':'var(--dn)')+'">'+(s.c>=0?'+':'')+s.c.toFixed(1)+'%</span><span>Vol:'+fmt(s.v)+'</span></div>'
      +'<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:4px;margin-bottom:6px;font-family:var(--fm);font-size:9px;text-align:center">'
      +'<div style="padding:4px;background:var(--bg2);border-radius:6px"><div style="color:var(--neon);font-weight:700">'+fP(s.entry)+'</div><div style="font-size:7px;color:var(--t3)">Entry</div></div>'
      +'<div style="padding:4px;background:var(--bg2);border-radius:6px"><div style="color:var(--up);font-weight:700">'+fP(s.target)+'</div><div style="font-size:7px;color:var(--t3)">Target</div></div>'
      +'<div style="padding:4px;background:var(--bg2);border-radius:6px"><div style="color:var(--dn);font-weight:700">'+fP(s.stop)+'</div><div style="font-size:7px;color:var(--t3)">Stop</div></div>'
      +'<div style="padding:4px;background:var(--bg2);border-radius:6px"><div style="color:'+(s.rr>=2.5?'var(--up)':'var(--warn)')+';font-weight:700">1:'+s.rr+'</div><div style="font-size:7px;color:var(--t3)">R:R</div></div></div>';
    if(s.reasons.length){h+='<div style="margin-bottom:6px">';s.reasons.slice(0,4).forEach(function(r){h+='<div style="font-size:8px;display:flex;align-items:center;gap:4px;margin-bottom:2px"><span>'+r.ic+'</span><span style="color:var(--t1)">'+r.t+'</span></div>'});h+='</div>'}
    h+='<div style="height:4px;background:var(--bg2);border-radius:2px;overflow:hidden;margin-bottom:6px"><div style="width:'+s.conf+'%;height:100%;background:'+(s.ultra?'linear-gradient(90deg,var(--ultra),var(--dn))':s.conf>=50?'var(--up)':'var(--warn)')+';border-radius:2px"></div></div>'
      +'<div style="display:flex;gap:6px;font-size:8px"><button class="chart-tf" onclick="openCoin(\''+s.s+'\')">📈 Chart</button><span style="font-size:8px;color:var(--t3);padding:4px">⏱ '+s.dur+'</span></div></div>'});
