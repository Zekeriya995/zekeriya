...
var adminTapCount=0;var adminTapTimer=null;
function openAdminPanel(){}
function renderAdminPanel(){try{renderMonPanel()}catch(e){}}
/* ═══ END MONITOR ═══ */
/* CACHE */
...
/* ═══ TAB 2: SMART TRADING ═══ */
async function loadTrading(){var trLoadEl=document.getElementById('tradeList');if(trLoadEl)trLoadEl.innerHTML='<div class="ldr"><div class="ldr-d"></div><div class="ldr-d"></div><div class="ldr-d"></div></div>';var c=quickScan();var r=await deepAnalyze(c);cache.scan=r;cache.scanTime=Date.now();var sigs=[];
...
/* ═══ TAB 3: SMALL CAPS ═══ */
async function loadSmallCapsUI(){document.getElementById('smallList').innerHTML='<div class="empty"><div class="empty-ic">🚫</div><div class="empty-tx">'+(lang==='ar'?'💎 صيد الجواهر معطّل حالياً — التركيز على Top 100 فقط':'💎 Gem Hunter disabled — Focus on Top 100 only')+'</div></div>';}
...
/* WS */
function initWS(){/* disabled — data from PROXY /api/all */}
/* ═══ 🔌 FEATURE 1: MULTI-STREAM WEBSOCKET ═══ */
var wsAgg=null,wsLiq=null,wsDepth=null,liquidationData={},depthSnapshots={};
function initAggTradeWS(){/* disabled — data from PROXY /api/all */}
function initLiqWS(){/* disabled — data from PROXY /api/all */}
function initDepthWS(){/* disabled — data from PROXY /api/all */}
...
function updateQACards(){
  try{
    ...
    var monEl=document.getElementById('qaMon');if(monEl){var q=getConnQuality();monEl.textContent=q+'%';monEl.style.color=q>=80?'var(--up)':q>=50?'var(--warn)':'var(--dn)'}
  }catch(e){}
}
/* 🗺️ HEATMAP */
...
function renderAlerts(){
  ...
  el.innerHTML=hist.slice(0,30).map(function(n){
    ...
    return'<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--glass);border:1px solid var(--bdr);border-radius:10px;margin-bottom:4px'+(age<300000?';border-color:rgba(0,255,136,.1)':'')+'">'
      +'<span style="font-size:16px;min-width:24px;text-align:center">'+(n.icon||n.ic||'🔔')+'</span>'
      +'<div style="flex:1"><div style="font-size:10px;color:var(--t0);font-weight:700">'+n.sym+' — '+n.type+'</div><div style="font-size:8px;color:var(--t3)">'+(n.body||n.detail||'')+'</div></div>'
...
function renderMonPanel(){
  ...
  var ms=monitorState||{perf:{overallRate:0,totalTrades:0,totalWins:0,totalLosses:0,bestHour:-1,worstHour:-1},factorStats:{},confCalib:{},hourStats:{},coinStats:{},coinBlacklist:[],weights:{},minConf:55,failPatterns:[]};
  ...
  var tkC=Object.keys(T).length;var frC=Object.keys(FR).length;var oiC=Object.keys(OI).length;
  var proxyOk=tkC>100;var q=proxyOk?Math.min(100,Math.round((tkC>0?20:0)+(frC>0?20:0)+(oiC>0?20:0)+(Object.keys(LS).length>0?20:0)+(apiRate>=80?20:10))):0;
  var qCol=q>=80?'var(--up)':q>=50?'var(--warn)':'var(--dn)';
  var qLabel=q>=80?(ar?'ممتاز':'Excellent'):q>=50?(ar?'مقبول':'OK'):(ar?'ضعيف':'Poor');
  /* ═══ 1. HEALTH SCORE ═══ */
  h+='<div style="text-align:center;padding:16px;background:var(--glass);border:1.5px solid rgba(56,72,96,.1);border-radius:16px;margin-bottom:12px">';
  h+='<div style="font-family:var(--fm);font-size:48px;font-weight:800;color:'+qCol+';line-height:1">'+q+'%</div>';
  h+='<div style="font-size:13px;font-weight:700;color:'+qCol+';margin-top:4px">'+(ar?'صحة المنصة — '+qLabel:'Platform Health — '+qLabel)+'</div>';
  h+='<div style="height:6px;background:rgba(56,72,96,.15);border-radius:3px;margin:10px 0 0;overflow:hidden"><div style="height:100%;width:'+q+'%;border-radius:3px;background:'+qCol+'"></div></div></div>';
  /* ═══ 2. PROXY SERVER STATUS ═══ */
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">📡 '+(ar?'سيرفر البيانات':'Data Proxy Server')+'</div>';
  h+='<div class="cd" style="padding:10px;font-size:11px">';
  h+='<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr)"><span style="color:var(--t2)">'+(ar?'الحالة':'Status')+'</span><span style="font-weight:700;color:'+(proxyOk?'var(--up)':'var(--dn)')+'">'+(proxyOk?'✅ '+(ar?'متصل':'Connected'):'❌ '+(ar?'غير متصل':'Disconnected'))+'</span></div>';
  h+='<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr)"><span style="color:var(--t2)">'+(ar?'المصدر':'Source')+'</span><span style="font-family:var(--fm);font-weight:700;color:var(--neon)">Proxy (3 '+(ar?'بورصات':'exchanges')+')</span></div>';
  ...
  h+='<div style="display:flex;justify-content:space-between;padding:5px 0"><span style="color:var(--t2)">'+(ar?'طريقة الاتصال':'Method')+'</span><span style="font-family:var(--fm);font-weight:700">1 call / 5s</span></div></div>';
  /* ═══ 3. STATUS GRID ═══ */
  h+='<div style="display:grid;grid-template-columns:repeat(2,1fr);gap:6px;margin:12px 0">';
  h+='<div class="cd" style="padding:12px;text-align:center"><div style="font-family:var(--fm);font-size:20px;font-weight:800;color:var(--neon)">'+tkC+'</div><div style="font-size:10px;color:var(--t2)">'+(ar?'عملات':'Coins')+'</div><div style="font-size:8px;color:var(--t3)">Binance+Bybit</div></div>';
  h+='<div class="cd" style="padding:12px;text-align:center"><div style="font-family:var(--fm);font-size:20px;font-weight:800;color:'+(frC>10?'var(--up)':'var(--warn)')+'">'+frC+'</div><div style="font-size:10px;color:var(--t2)">FR</div><div style="font-size:8px;color:var(--t3)">'+(ar?'3 بورصات مدمج':'3 exchanges merged')+'</div></div>';
  h+='<div class="cd" style="padding:12px;text-align:center"><div style="font-size:20px">'+mktIc+'</div><div style="font-size:10px;color:var(--t2)">'+(ar?'حالة السوق':'Market')+'</div><div style="font-size:9px;color:'+mktCol+'">'+mkt.level+'</div></div>';
  h+='<div class="cd" style="padding:12px;text-align:center"><div style="font-family:var(--fm);font-size:20px;font-weight:800;color:'+perfCol+'">'+perf.overallRate+'%</div><div style="font-size:10px;color:var(--t2)">'+(ar?'نسبة النجاح':'Win Rate')+'</div><div style="font-size:8px;color:var(--t3)">'+perf.totalWins+'/'+perf.totalTrades+'</div></div></div>';
  /* ═══ 4. LIVE DIAGNOSIS ═══ */
  var diagOk=0;
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">🩺 '+(ar?'تشخيص لحظي':'Live Diagnosis')+'</div>';
  function dR(ic,nm,det,st,ok){diagOk+=ok?1:0;var bdr=ok?'rgba(0,255,136,.08)':st==='واقف'||st==='Stopped'?'rgba(255,56,96,.1)':'rgba(255,184,0,.08)';var sCol=ok?'var(--up)':st==='واقف'||st==='Stopped'?'var(--dn)':'var(--warn)';var sBg=ok?'rgba(0,255,136,.1)':st==='واقف'||st==='Stopped'?'rgba(255,56,96,.1)':'rgba(255,184,0,.1)';return'<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--glass);border:1px solid '+bdr+';border-radius:10px;margin-bottom:4px"><span style="font-size:16px;min-width:24px;text-align:center">'+ic+'</span><div style="flex:1"><div style="font-size:11px;font-weight:700;color:var(--t0)">'+nm+'</div><div style="font-size:9px;color:var(--t3)">'+det+'</div></div><span style="font-size:9px;font-weight:700;padding:3px 8px;border-radius:5px;background:'+sBg+';color:'+sCol+'">'+st+'</span></div>';}
  h+=dR('📡',ar?'سيرفر Proxy':'Proxy Server','Cloudflare Tunnel',proxyOk?(ar?'متصل':'Connected'):(ar?'منقطع':'Down'),proxyOk);
  h+=dR('📊',ar?'بيانات الأسعار':'Prices',tkC+' '+(ar?'عملة':'coins'),tkC>100?(ar?'سليم':'OK'):(ar?'ضعيف':'Low'),tkC>100);
  h+=dR('💰','Funding Rate',frC+' '+(ar?'عملة — 3 بورصات':'coins — 3 exchanges'),frC>10?(ar?'سليم':'OK'):(ar?'ناقص':'Low'),frC>10);
  h+=dR('📈','Open Interest',oiC+' '+(ar?'عملة مدمج':'coins merged'),oiC>3?(ar?'سليم':'OK'):(ar?'ناقص':'Low'),oiC>3);
  var lsC=Object.keys(LS).length;
  h+=dR('📊','Long/Short',lsC+' '+(ar?'عملة':'coins'),lsC>0?(ar?'سليم':'OK'):(ar?'ناقص':'Low'),lsC>0);
  var liqC=liqEvents?liqEvents.filter(function(e){return e.time&&Date.now()-e.time<3600000}).length:0;
  h+=dR('💥',ar?'تصفيات':'Liquidations',liqC+' '+(ar?'آخر ساعة':'last hr'),liqC>0?(ar?'سليم':'OK'):(ar?'بطيء':'Slow'),liqC>0);
  var whC=Object.keys(whaleWaves).filter(function(s){return whaleWaves[s]&&whaleWaves[s].waves&&whaleWaves[s].waves.length}).length;
  h+=dR('🐋',ar?'حيتان':'Whales',whC+' '+(ar?'نشطة':'active'),whC>0?(ar?'سليم':'OK'):(ar?'لا نشاط':'None'),whC>0);
  var depC=Object.keys(depthSnapshots).filter(function(s){return depthSnapshots[s]&&depthSnapshots[s].bids&&depthSnapshots[s].bids.length}).length;
  h+=dR('📕','Order Book',depC+' '+(ar?'عملة':'coins'),depC>0?(ar?'سليم':'OK'):(ar?'واقف':'Stopped'),depC>0);
  var tkDC=Object.keys(takerData).filter(function(s){return takerData[s]}).length;
  h+=dR('⚡','Taker',tkDC+' '+(ar?'عملة':'coins'),tkDC>0?(ar?'سليم':'OK'):(ar?'ناقص':'Low'),tkDC>0);
  /* ═══ 5. SECTION HEALTH ═══ */
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">📱 '+(ar?'صحة الأقسام':'Sections')+' <span style="font-size:9px;padding:2px 8px;border-radius:5px;background:rgba(0,255,136,.1);color:var(--up)">'+diagOk+'/9</span></div>';
  function sR(ic,nm,det,pct){var col=pct>=90?'var(--up)':pct>=60?'var(--warn)':'var(--dn)';return'<div style="display:flex;align-items:center;gap:8px;padding:8px 10px;background:var(--glass);border:1px solid '+(pct>=90?'rgba(0,255,136,.08)':'rgba(255,184,0,.08)')+';border-radius:10px;margin-bottom:4px"><span style="font-size:14px;min-width:22px;text-align:center">'+ic+'</span><div style="flex:1"><div style="font-size:11px;font-weight:700;color:var(--t0)">'+nm+'</div><div style="font-size:9px;color:var(--t3)">'+det+'</div></div><span style="font-family:var(--fm);font-size:11px;font-weight:700;color:'+col+'">'+pct+'%</span></div>';}
  h+=sR('🏠',ar?'الرئيسية':'Home','topCoins+QA+Top3+L/S',tkC>0?100:0);
  h+=sR('📡',ar?'السكانر':'Scanner','quickScan+deepAnalyze',tkC>0?100:50);
  h+=sR('🐋',ar?'الحيتان':'Whales','waves+sells+gems',whC>0?100:60);
  h+=sR('📈',ar?'حركة السوق':'Market','BTC+ETH 12 sec',tkC>0?100:0);
  var indPct=Math.round(((frC>0?1:0)+(oiC>0?1:0)+(liqC>0?1:0)+(whC>0?1:0)+(tkDC>0?1:0)+(depC>0?1:0))*100/7);
  h+=sR('📊',ar?'المؤشرات':'Indicators','7 cards',indPct);
  h+=sR('👤',ar?'حسابي':'Profile','5 tabs',100);
  /* ═══ 6. VALIDATOR LOG ═══ */
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">🔍 '+(ar?'سجل الفحص':'Validator Log')+'</div>';
  h+='<div class="cd" style="padding:10px;max-height:120px;overflow-y:auto">';
  if(typeof validatorLog!=='undefined'&&validatorLog.length){validatorLog.slice(0,10).forEach(function(l){var a=typeof timeAgo!=='undefined'?timeAgo(l.time):{text:''};h+='<div style="display:flex;gap:6px;padding:4px 0;border-bottom:1px solid var(--bdr);font-size:9px"><span>'+l.type+'</span><span style="flex:1;color:var(--t1)">'+l.msg+'</span><span style="color:var(--t3);font-family:var(--fm);font-size:7px">'+a.text+'</span></div>';});}else{h+='<div style="text-align:center;color:var(--t3);font-size:10px;padding:6px">'+(ar?'لم يتم الفحص بعد':'Not scanned yet')+'</div>';}
  h+='</div>';
  /* ═══ 7. FACTOR WEIGHTS ═══ */
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">⚖️ '+(ar?'أوزان العوامل':'Factor Weights')+'</div><div class="cd" style="padding:10px">';
  var fK=Object.keys(ms.factorStats);fK.sort(function(a,b){return(ms.factorStats[b].winRate||0)-(ms.factorStats[a].winRate||0)});
  if(fK.length){fK.forEach(function(k){var f=ms.factorStats[k];var wr=f.winRate||0;var wC=wr>=70?'var(--up)':wr>=50?'var(--warn)':'var(--dn)';h+='<div style="display:flex;align-items:center;padding:5px 0;border-bottom:1px solid var(--bdr);font-size:11px"><span style="font-weight:700;min-width:55px">'+k+'</span><div style="flex:1;height:5px;background:rgba(56,72,96,.1);border-radius:3px;margin:0 8px;overflow:hidden"><div style="height:100%;width:'+Math.min(100,wr)+'%;border-radius:3px;background:'+wC+'"></div></div><span style="font-family:var(--fm);font-weight:700;color:'+wC+';min-width:35px;text-align:left">'+wr+'%</span><span style="font-family:var(--fm);font-size:9px;color:var(--t3);min-width:40px">'+f.wins+'W/'+f.total+'T</span></div>';});}else{h+='<div style="color:var(--t3);font-size:11px;text-align:center;padding:8px">'+(ar?'تحتاج صفقات':'Need trades')+'</div>';}
  h+='</div>';
  /* ═══ 8. CONFIDENCE ═══ */
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">🎯 '+(ar?'معايرة الثقة':'Confidence')+'</div><div class="cd" style="padding:10px">';
  var bk=Object.keys(ms.confCalib).sort();
  if(bk.length){bk.forEach(function(b){var c=ms.confCalib[b];var cl=c.realRate>=65?'var(--up)':c.realRate>=45?'var(--warn)':'var(--dn)';h+='<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr);font-size:11px"><span style="color:var(--t2)">'+(ar?'ثقة ':'Conf ')+b+'%</span><span style="font-family:var(--fm);font-weight:700;color:'+cl+'">'+c.realRate+'%</span><span style="font-size:9px;color:var(--t3)">('+c.wins+'/'+c.total+')</span></div>';});}else{h+='<div style="color:var(--t3);font-size:11px;text-align:center;padding:8px">'+(ar?'تحتاج 5+ صفقات':'Need 5+ trades')+'</div>';}
  h+='</div>';
  /* ═══ 9. HOURS ═══ */
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">🕐 '+(ar?'الساعات':'Hours')+'</div><div class="cd" style="padding:10px"><div style="display:flex;flex-wrap:wrap;gap:2px">';
  for(var hr=0;hr<24;hr++){var hs=ms.hourStats[String(hr)];var rate=hs?hs.rate:-1;var bg=rate<0?'var(--bg2)':rate>=65?'rgba(0,255,136,.15)':rate>=45?'rgba(255,184,0,.15)':'rgba(255,56,96,.15)';var cl=rate<0?'var(--t3)':rate>=65?'var(--up)':rate>=45?'var(--warn)':'var(--dn)';h+='<div style="width:26px;padding:3px 1px;background:'+bg+';border-radius:4px;text-align:center"><div style="font-size:7px;color:var(--t3)">'+String(hr).padStart(2,'0')+'</div><div style="font-size:8px;font-weight:700;font-family:var(--fm);color:'+cl+'">'+(rate>=0?rate+'%':'--')+'</div></div>';}
  h+='</div>';if(perf.bestHour>=0)h+='<div style="font-size:10px;margin-top:6px;color:var(--t2)">🏆 '+(ar?'أفضل: ':'Best: ')+'<b style="color:var(--up)">'+perf.bestHour+':00</b>'+(perf.worstHour>=0?' — '+(ar?'أسوأ: ':'Worst: ')+'<b style="color:var(--dn)">'+perf.worstHour+':00</b>':'')+'</div>';h+='</div>';
  /* ═══ 10. COINS ═══ */
  var coins=Object.entries(ms.coinStats).sort(function(a,b){return(b[1].rate||0)-(a[1].rate||0)});
  if(coins.length){h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">🪙 '+(ar?'أداء العملات':'Coins')+'</div><div class="cd" style="padding:10px">';coins.forEach(function(e){var s=e[0],c=e[1];var cl=c.rate>=65?'var(--up)':c.rate>=45?'var(--warn)':'var(--dn)';h+='<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr);font-size:11px"><span style="font-weight:700">'+s+'</span><span style="font-family:var(--fm);font-weight:700;color:'+cl+'">'+c.rate+'%</span><span style="font-size:9px;color:var(--t3)">'+c.wins+'/'+c.total+'</span></div>';});h+='</div>';}
  /* ═══ 11. FAIL PATTERNS ═══ */
  if(ms.failPatterns&&ms.failPatterns.length){h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">⚠️ '+(ar?'أنماط فشل':'Fail Patterns')+'</div><div class="cd" style="padding:10px">';ms.failPatterns.slice(0,5).forEach(function(p){h+='<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr);font-size:11px"><span>'+p.label+'</span><span style="font-family:var(--fm);font-weight:700;color:var(--dn)">'+p.failRate+'%</span></div>';});h+='</div>';}
  /* ═══ 12. BLACKLIST ═══ */
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">🚫 '+(ar?'محظورة':'Blacklist')+'</div>';
  ...
  /* ═══ 13. SYSTEM ═══ */
  h+='<div style="font-weight:800;font-size:13px;color:var(--t1);margin:14px 0 8px">🏥 '+(ar?'النظام':'System')+'</div><div class="cd" style="padding:10px;font-size:11px">';
  h+='<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr)"><span style="color:var(--t2)">'+(ar?'عملات':'Coins')+'</span><span style="font-family:var(--fm);font-weight:700;color:var(--neon)">'+tkC+'</span></div>';
  h+='<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr)"><span style="color:var(--t2)">FR/OI/LS</span><span style="font-family:var(--fm);font-weight:700">'+frC+'/'+oiC+'/'+Object.keys(LS).length+'</span></div>';
  h+='<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid var(--bdr)"><span style="color:var(--t2)">'+(ar?'حد ثقة':'Min Conf')+'</span><span style="font-family:var(--fm);font-weight:700;color:var(--warn)">'+ms.minConf+'%</span></div>';
  h+='<div style="display:flex;justify-content:space-between;padding:5px 0"><span style="color:var(--t2)">'+(ar?'مصدر البيانات':'Data Source')+'</span><span style="font-family:var(--fm);font-weight:700;color:var(--neon)">Proxy Server</span></div></div>';
  el.innerHTML=h;
}
/* DASHBOARD */
...
async function init(){try{document.getElementById('sInp').placeholder=t('search_ph')}catch(e){}try{document.getElementById('notifB').dataset.c='0'}catch(e){}
  ...
  // === BOOT ALL 15 NEW FEATURES ===
  try { bootNewFeatures(); } catch(e) { console.error('bootNewFeatures:', e); }
}
/* ═══════════════════════════════════════════════════════════════
   NEXUS PRO V10 — 15 NEW FEATURES (Phase 1 + 2 + 3)
   Insert this block BEFORE init() in app.js
   All code: var only, no template literals, no arrow functions,
   no optional chaining, bilingual ar/en, null-safe getElementById
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══ P1-1: MTF CONFLUENCE — Multi-Timeframe alignment ═══ */
var mtfCache={};
async function calcMTF(sym){
  var ck=mtfCache[sym];if(ck&&Date.now()-ck.t<300000)return ck.d;
  try{
    var res=await Promise.all([
      fj(BN+'/klines?symbol='+sym+'USDT&interval=1h&limit=30'),
      fj(BN+'/klines?symbol='+sym+'USDT&interval=4h&limit=30'),
      fj(BN+'/klines?symbol='+sym+'USDT&interval=1d&limit=20'),
      fj(BN+'/klines?symbol='+sym+'USDT&interval=1w&limit=10')
    ]);
    var frames=['1H','4H','1D','1W'];var aligned=0;var details=[];
    res.forEach(function(kl,idx){
      if(!kl||kl.length<10){details.push({tf:frames[idx],dir:'--',ok:false});return}
      var c=kl.map(function(k){return+k[4]});
      var ema10=calcEMA(c.slice(-10),10);var ema20=calcEMA(c,20);
      var rsi=calcRSI(c);var macd=calcMACD(c);
      var bull=c[c.length-1]>ema10&&ema10>ema20&&macd.h>0;
      var bear=c[c.length-1]<ema10&&ema10<ema20&&macd.h<0;
      var dir=bull?'BULL':bear?'BEAR':'NEUTRAL';
      var ok=bull;aligned+=bull?1:0;
      details.push({tf:frames[idx],dir:dir,ok:ok,rsi:Math.round(rsi),macdH:macd.h>0})
    });
    var score=Math.round(aligned/4*100);
    var signal=score>=75?(lang==='ar'?'🟢 توافق قوي':'🟢 Strong alignment'):score>=50?(lang==='ar'?'🟡 توافق جزئي':'🟡 Partial'):score>=25?(lang==='ar'?'🟠 ضعيف':'🟠 Weak'):(lang==='ar'?'🔴 لا توافق':'🔴 No alignment');
    var d={sym:sym,score:score,aligned:aligned,total:4,signal:signal,details:details,time:Date.now()};
    mtfCache[sym]={d:d,t:Date.now()};return d;
  }catch(e){return{sym:sym,score:0,aligned:0,total:4,signal:'--',details:[],time:Date.now()}}
}
function renderMTFPanel(){
  var el=document.getElementById('mtfPanel');if(!el)return;
  var ar=lang==='ar';
  var syms=['BTC','ETH','SOL','BNB','XRP'];
  el.innerHTML='<div class="ldr"><div class="ldr-d"></div><div class="ldr-d"></div><div class="ldr-d"></div></div>';
  Promise.all(syms.map(function(s){return calcMTF(s)})).then(function(results){
    var h='';
    results.forEach(function(r){
      if(!r||!r.details)return;
      var col=r.score>=75?'var(--up)':r.score>=50?'var(--warn)':'var(--dn)';
      h+='<div class="cd" style="padding:10px;margin-bottom:6px" onclick="openCoin(\''+r.sym+'\')">';
      h+='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">';
      h+='<span style="font-family:var(--fd);font-weight:800;font-size:13px">'+r.sym+'</span>';
      h+='<span style="font-family:var(--fm);font-weight:800;color:'+col+'">'+r.score+'% ('+r.aligned+'/4)</span></div>';
      h+='<div style="display:flex;gap:4px">';
      r.details.forEach(function(d){
        var bg=d.dir==='BULL'?'var(--ud)':d.dir==='BEAR'?'var(--dd)':'var(--bg2)';
        var fc=d.dir==='BULL'?'var(--up)':d.dir==='BEAR'?'var(--dn)':'var(--t3)';
        var ic=d.dir==='BULL'?'↑':d.dir==='BEAR'?'↓':'→';
        h+='<div style="flex:1;text-align:center;padding:4px;background:'+bg+';border-radius:6px">';
        h+='<div style="font-size:8px;color:var(--t3)">'+d.tf+'</div>';
        h+='<div style="font-size:11px;font-weight:800;color:'+fc+'">'+ic+'</div></div>';
      });
      h+='</div>';
      h+='<div style="font-size:9px;color:'+col+';text-align:center;margin-top:4px">'+r.signal+'</div>';
      h+='</div>';
    });
    if(el)el.innerHTML=h||'<div class="muted">'+(ar?'لا بيانات':'No data')+'</div>';
  });
}

/* ═══ P1-2: LIVE PnL DASHBOARD ═══ */
function renderLivePnL(){
  var el=document.getElementById('livePnL');if(!el)return;
  var ar=lang==='ar';
  var open=activeTrades.filter(function(t){return t.status==='OPEN'});
  if(!open.length){el.innerHTML='<div style="text-align:center;font-size:10px;color:var(--t3);padding:8px">'+(ar?'لا صفقات مفتوحة':'No open trades')+'</div>';return}
  var totalPnl=0;var totalVal=0;
  open.forEach(function(tr){var d=T[tr.sym];if(!d)return;tr.pnl=(d.p-tr.entry)/tr.entry*100;totalPnl+=tr.pnl;totalVal+=d.p});
  var avgPnl=open.length>0?totalPnl/open.length:0;
  var col=avgPnl>=0?'var(--up)':'var(--dn)';
  var h='<div style="display:flex;justify-content:space-between;align-items:center">';
  h+='<div><span style="font-size:10px;color:var(--t2)">'+(ar?'صفقات مفتوحة':'Open')+'</span>';
  h+='<div style="font-family:var(--fm);font-size:18px;font-weight:800;color:'+col+';direction:ltr">'+(avgPnl>=0?'+':'')+avgPnl.toFixed(1)+'%</div></div>';
  h+='<div style="display:flex;gap:3px">';
  open.forEach(function(tr){
    var pc=tr.pnl>=0?'var(--up)':'var(--dn)';
    h+='<div style="text-align:center;padding:3px 6px;background:var(--bg2);border-radius:6px;min-width:36px">';
    h+='<div style="font-size:7px;font-weight:700">'+tr.sym+'</div>';
    h+='<div style="font-family:var(--fm);font-size:8px;font-weight:800;color:'+pc+';direction:ltr">'+(tr.pnl>=0?'+':'')+tr.pnl.toFixed(1)+'%</div></div>';
  });
  h+='</div></div>';
  el.innerHTML=h;
}

/* ═══ P1-3: SMART MULTI-FACTOR TG ALERTS ═══ */
function checkSmartAlerts(){
  var ar=lang==='ar';
  WL.slice(0,30).forEach(function(sym){
    var d=T[sym];if(!d||!d.p)return;
    var factors=0;var reasons=[];
    var fr=FR[sym];if(fr&&fr.rate<-0.02){factors++;reasons.push('FR-')}
    var ww=whaleWaves[sym];if(ww&&ww.engine&&ww.engine.confidence>=50){factors++;reasons.push('Whale'+ww.engine.confidence+'%')}
    var ls=LS[sym];if(ls&&ls.short>=60){factors++;reasons.push('Short '+ls.short.toFixed(0)+'%')}
    if(d.c>=2&&d.c<8&&d.v>1e8){factors++;reasons.push('Vol+Move')}
    var cvd=analyzeCVD(sym);if(cvd.divergence==='BULLISH'){factors++;reasons.push('CVD+')}
    if(factors>=3){
      var k='smart_'+sym+'_'+new Date().getHours();
      if(!notifiedSet[k]){
        notifiedSet[k]=true;try{localStorage.setItem('nxnot10',JSON.stringify(notifiedSet))}catch(e){}
        var msg='🎯 <b>SMART ALERT — '+sym+'/USDT</b>\n\n'
          +'💰 <b>'+fP(d.p)+'</b> ('+(d.c>=0?'+':'')+d.c.toFixed(1)+'%)\n'
          +factors+' '+(ar?'عوامل متوافقة':'confluence factors')+':\n'
          +reasons.join(' | ')+'\n\n'
          +'📍 NEXUS PRO v10';
        sendTG(msg);
        showPopup('🎯',sym+' — '+(ar?'تنبيه ذكي!':'Smart Alert!'),factors+' factors: '+reasons.join(', '));
        addNotifHist('🎯',sym,ar?'تنبيه ذكي':'Smart Alert',reasons.join(', '));
      }
    }
  });
}

/* ═══ P1-4: BTC CORRELATION MAP ═══ */
function calcCorrelation(arr1,arr2){
  if(!arr1||!arr2||arr1.length<5||arr2.length<5)return 0;
  var n=Math.min(arr1.length,arr2.length);var a=arr1.slice(-n),b=arr2.slice(-n);
  var ma=a.reduce(function(s,x){return s+x},0)/n;var mb=b.reduce(function(s,x){return s+x},0)/n;
  var num=0,da=0,db=0;
  for(var i=0;i<n;i++){var xa=a[i]-ma;var xb=b[i]-mb;num+=xa*xb;da+=xa*xa;db+=xb*xb}
  return da>0&&db>0?num/Math.sqrt(da*db):0;
}
function renderCorrelationMap(){
  var el=document.getElementById('corrMap');if(!el)return;
  var ar=lang==='ar';
  var btcH=sparkHist['BTC'];if(!btcH||btcH.length<5){el.innerHTML='<div class="muted">'+(ar?'تحتاج بيانات أكثر':'Need more data')+'</div>';return}
  var syms=['ETH','SOL','BNB','XRP','ADA','DOGE','LINK','AVAX','DOT','NEAR'];
  var results=[];
  syms.forEach(function(s){
    var sh=sparkHist[s];if(!sh||sh.length<5)return;
    var corr=calcCorrelation(btcH,sh);
    results.push({s:s,corr:corr,change:T[s]?T[s].c:0});
  });
  results.sort(function(a,b){return a.corr-b.corr});
  var h='';
  results.forEach(function(r){
    var abs=Math.abs(r.corr);var col=abs>0.8?'var(--up)':abs>0.5?'var(--warn)':'var(--dn)';
    var lbl=abs>0.8?(ar?'مرتبط':'Corr'):abs>0.5?(ar?'متوسط':'Mid'):(ar?'مستقل — ألفا!':'Decorr — Alpha!');
    var w=Math.round(abs*100);
    h+='<div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid var(--bdr);font-size:10px">';
    h+='<span style="font-weight:700;min-width:36px">'+r.s+'</span>';
    h+='<div style="flex:1;height:5px;background:var(--bg2);border-radius:3px;overflow:hidden"><div style="width:'+w+'%;height:100%;background:'+col+';border-radius:3px"></div></div>';
    h+='<span style="font-family:var(--fm);font-weight:700;color:'+col+';min-width:40px;direction:ltr">'+(r.corr>=0?'+':'')+r.corr.toFixed(2)+'</span>';
    h+='<span style="font-size:8px;color:'+col+'">'+lbl+'</span></div>';
  });
  el.innerHTML=h||'<div class="muted">--</div>';
}

/* ═══ P1-5: AUTO POSITION SIZE from Signal ═══ */
function autoFillCalc(sym,entry,stop,target){
  try{
    var eEl=document.getElementById('rcEntry2');if(eEl)eEl.value=entry.toFixed(2);
    var sEl=document.getElementById('rcSL2');if(sEl)sEl.value=stop.toFixed(2);
    var tEl=document.getElementById('rcTP2');if(tEl)tEl.value=target.toFixed(2);
    calcRisk2();
    openQA('calc');
  }catch(e){}
}

/* ═══ P1-6: ATR TRAILING STOPS ═══ */
function calcATR(klines,period){
  if(!klines||klines.length<period+1)return 0;
  var trs=[];
  for(var i=1;i<klines.length;i++){
    var h=+klines[i][2],l=+klines[i][3],pc=+klines[i-1][4];
...
async function getATR(sym){
  var ck=atrCache[sym];if(ck&&Date.now()-ck.t<120000)return ck.v;
  try{
    var kl=await fj(BN+'/klines?symbol='+sym+'USDT&interval=1h&limit=20');
    var atr=calcATR(kl,14);atrCache[sym]={v:atr,t:Date.now()};return atr;
  }catch(e){return 0}
}
/* Enhanced monitorTrades with ATR trailing — patches existing function */
var origMonitorTrades=monitorTrades;
monitorTrades=function(){
  var open=activeTrades.filter(function(t){return t.status==='OPEN'});
  open.forEach(function(tr){
    if(tr.t1Hit&&!tr.atrTrail){
      getATR(tr.sym).then(function(atr){
        if(atr>0){tr.atrTrail=true;tr.trailingStop=Math.max(tr.trailingStop||0,tr.maxGainPrice-atr*2)}
      });
    }
    if(tr.atrTrail&&tr.maxGainPrice){
      getATR(tr.sym).then(function(atr){
        if(atr>0){var newStop=tr.maxGainPrice-atr*2;if(newStop>tr.trailingStop)tr.trailingStop=newStop}
      });
    }
  });
  origMonitorTrades();
...
/* ═══ P2-1: LIQUIDATION HEATMAP — Magnetic price zones ═══ */
function buildLiqHeatmap(sym){
  var d=T[sym];if(!d)return null;
  var events=(liqEvents||[]).filter(function(e){return(e.s===sym+'USDT'||e.s===sym)&&e.p>0});
  if(!events.length)return null;
  var price=d.p;var range=price*0.05;
  var zones={};var step=range/20;
  for(var z=price-range;z<=price+range;z+=step){
    var zKey=Math.round(z*100)/100;zones[zKey]=0;
    events.forEach(function(e){if(Math.abs(e.p-z)<step)(zones[zKey]+=(e.q||1)*(e.p||0))});
  }
  var sorted=Object.entries(zones).sort(function(a,b){return b[1]-a[1]});
  var maxV=sorted.length?sorted[0][1]:1;
  return{sym:sym,price:price,zones:sorted.slice(0,10).filter(function(z){return z[1]>0}),maxV:maxV};
}
function renderLiqHeatmapCard(){
  var el=document.getElementById('liqHeatmap');if(!el)return;
  var ar=lang==='ar';
  var data=buildLiqHeatmap('BTC');
  if(!data||!data.zones.length){el.innerHTML='<div class="muted">'+(ar?'لا بيانات تصفيات':'No liq data')+'</div>';return}
  var h='<div style="font-size:10px;font-weight:700;color:var(--t1);margin-bottom:6px">'+(ar?'مناطق مغناطيسية — BTC':'Magnetic zones — BTC')+'</div>';
  data.zones.forEach(function(z){
    var price=+z[0];var vol=z[1];var pct=data.maxV>0?vol/data.maxV*100:0;
    var above=price>data.price;
    var col=above?'var(--up)':'var(--dn)';var bg=above?'var(--ud)':'var(--dd)';
    var dist=((price-data.price)/data.price*100).toFixed(2);
    h+='<div style="display:flex;align-items:center;gap:4px;padding:3px 0;font-size:9px">';
    h+='<span style="font-family:var(--fm);min-width:55px;direction:ltr;color:'+col+'">'+fP(price)+'</span>';
    h+='<div style="flex:1;height:5px;background:var(--bg2);border-radius:3px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:'+col+';border-radius:3px"></div></div>';
    h+='<span style="font-family:var(--fm);min-width:35px;color:var(--t3);direction:ltr">'+(above?'+':'')+dist+'%</span></div>';
  });
  el.innerHTML=h;
}

/* ═══ P2-2: ORDER FLOW SCANNER — Pre-breakout detection ═══ */
function scanOrderFlow(){
  var results=[];
  var syms=WL.slice(0,20);
  syms.forEach(function(sym){
    var ds=depthSnapshots[sym];var d=T[sym];if(!ds||!ds.bids||!ds.bids.length||!d)return;
    var bidT=0,askT=0;
    try{
      ds.bids.slice(0,5).forEach(function(b){bidT+=((+b[0]||0)*(+b[1]||0))});
      ds.asks.slice(0,5).forEach(function(a){askT+=((+a[0]||0)*(+a[1]||0))});
    }catch(e){return}
    var ratio=askT>0?bidT/askT:1;
    if(ratio>=3||ratio<=0.33){
      results.push({s:sym,ratio:ratio,buy:ratio>1,bidT:bidT,askT:askT,price:d.p,change:d.c});
    }
  });
  return results.sort(function(a,b){return Math.abs(b.ratio)-Math.abs(a.ratio)});
}
function renderOrderFlow(){
  var el=document.getElementById('orderFlowList');if(!el)return;
  var ar=lang==='ar';var results=scanOrderFlow();
  if(!results.length){el.innerHTML='<div class="muted">'+(ar?'لا ضغط غير عادي':'No unusual pressure')+'</div>';return}
  var h='';
  results.slice(0,5).forEach(function(r){
    var col=r.buy?'var(--up)':'var(--dn)';
    var lbl=r.buy?(ar?'🟢 ضغط شراء هائل — انفجار قريب!':'🟢 Massive buy pressure — Breakout imminent!'):(ar?'🔴 ضغط بيع — حذر!':'🔴 Sell pressure — Caution!');
    h+='<div class="cd" style="padding:8px;margin-bottom:4px;border-left:3px solid '+col+'" onclick="openCoin(\''+r.s+'\')">';
    h+='<div style="display:flex;justify-content:space-between;align-items:center">';
    h+='<span style="font-weight:800;font-size:12px">'+r.s+'</span>';
    h+='<span style="font-family:var(--fm);font-weight:800;color:'+col+'">'+r.ratio.toFixed(1)+'x</span></div>';
    h+='<div style="font-size:9px;color:'+col+';margin-top:2px">'+lbl+'</div></div>';
  });
  el.innerHTML=h;
}

/* ═══ P2-3: ARBITRAGE DETECTOR ═══ */
function detectArbitrage(){
  var results=[];
  Object.keys(T).forEach(function(sym){
    var d=T[sym];if(!d||!d.p||!d.by)return;
    var spread=Math.abs(d.p-d.by)/d.p*100;
    if(spread>=0.3){
      var dir=d.by>d.p?'BY>BN':'BN>BY';
      results.push({s:sym,bn:d.p,by:d.by,spread:spread,dir:dir,vol:d.v});
    }
    if(CBP[sym]){
      var cbSpread=Math.abs(CBP[sym]-d.p)/d.p*100;
      if(cbSpread>=0.3){
        results.push({s:sym,bn:d.p,cb:CBP[sym],spread:cbSpread,dir:CBP[sym]>d.p?'CB>BN':'BN>CB',vol:d.v});
      }
    }
  });
  return results.sort(function(a,b){return b.spread-a.spread}).slice(0,10);
}
function renderArbitrage(){
  var el=document.getElementById('arbList');if(!el)return;
  var ar=lang==='ar';var results=detectArbitrage();
  if(!results.length){el.innerHTML='<div class="muted">'+(ar?'لا أربيتراج حالياً':'No arbitrage now')+'</div>';return}
  var h='';
  results.forEach(function(r){
    var col=r.spread>=0.8?'var(--up)':r.spread>=0.5?'var(--warn)':'var(--t2)';
    ...
    h+='<span style="font-weight:700;min-width:36px">'+r.s+'</span>';
    h+='<span style="font-family:var(--fm);color:var(--t2);direction:ltr">'+r.dir+'</span>';
    h+='<span style="font-family:var(--fm);font-weight:700;color:'+col+';direction:ltr">+'+r.spread.toFixed(2)+'%</span>';
    h+='<span style="font-size:8px;color:var(--t3)">'+fmt(r.vol)+'</span></div>';
  });
  el.innerHTML=h;
}

/* ═══ P2-4: MARKET REGIME ENGINE ═══ */
var marketRegime={type:'unknown',adx:0,bbw:0,volProfile:'normal',time:0};
async function detectMarketRegime(){
  try{
    var kl=await fj(BN+'/klines?symbol=BTCUSDT&interval=4h&limit=30');
    if(!kl||kl.length<20)return;
    var c=kl.map(function(k){return+k[4]});var h=kl.map(function(k){return+k[2]});var l=kl.map(function(k){return+k[3]});var v=kl.map(function(k){return+k[5]});
    /* ADX approximation */
    var pdm=0,ndm=0,tr=0;
    for(var i=1;i<Math.min(14,c.length);i++){
      var hd=h[i]-h[i-1];var ld=l[i-1]-l[i];
      pdm+=hd>ld&&hd>0?hd:0;ndm+=ld>hd&&ld>0?ld:0;
      tr+=Math.max(h[i]-l[i],Math.abs(h[i]-c[i-1]),Math.abs(l[i]-c[i-1]));
    }
    var pdi=tr>0?pdm/tr*100:0;var ndi=tr>0?ndm/tr*100:0;
    var adx=pdi+ndi>0?Math.abs(pdi-ndi)/(pdi+ndi)*100:0;
    /* Bollinger Bandwidth */
    var avg20=c.slice(-20).reduce(function(a,b){return a+b},0)/20;
    var variance=c.slice(-20).reduce(function(s,x){return s+Math.pow(x-avg20,2)},0)/20;
    var sd=Math.sqrt(variance);var bbw=avg20>0?(4*sd)/avg20*100:0;
    /* Volume profile */
    var avgV=v.slice(-14).reduce(function(a,b){return a+b},0)/14;
    var recentV=(v[v.length-1]+v[v.length-2])/2;
    var volRatio=avgV>0?recentV/avgV:1;
    /* Classify */
    var type;
    if(adx>30&&bbw>4){type='trending'}
    else if(adx<20&&bbw<2){type='quiet'}
    else if(adx<25&&bbw>3){type='volatile'}
    else{type='ranging'}
    marketRegime={type:type,adx:Math.round(adx),bbw:+bbw.toFixed(1),volProfile:volRatio>1.5?'high':volRatio<0.7?'low':'normal',volRatio:+volRatio.toFixed(1),time:Date.now()};
  }catch(e){}
}
function getRegimeLabel(){
  var ar=lang==='ar';var r=marketRegime;
  if(r.type==='trending')return{ic:'📈',lbl:ar?'ترند — الإشارات قوية':'Trending — Signals strong',col:'var(--up)',bg:'var(--ud)'};
  if(r.type==='ranging')return{ic:'↔️',lbl:ar?'عرضي — حذر من الاختراقات الكاذبة':'Ranging — Beware fakeouts',col:'var(--warn)',bg:'var(--wd)'};
  if(r.type==='volatile')return{ic:'⚡',lbl:ar?'متذبذب — وسّع الوقف':'Volatile — Widen stops',col:'var(--dn)',bg:'var(--dd)'};
  if(r.type==='quiet')return{ic:'😴',lbl:ar?'هادئ — انتظر':'Quiet — Wait',col:'var(--t2)',bg:'var(--bg2)'};
  return{ic:'❓',lbl:'--',col:'var(--t3)',bg:'var(--bg2)'};
}
function renderRegimeBadge(){
  var el=document.getElementById('regimeBadge');if(!el)return;
  var r=getRegimeLabel();
  el.innerHTML='<div style="display:flex;align-items:center;gap:6px;padding:6px 10px;background:'+r.bg+';border-radius:8px">';
  el.innerHTML+='<span style="font-size:14px">'+r.ic+'</span>';
  el.innerHTML+='<div><div style="font-size:10px;font-weight:700;color:'+r.col+'">'+r.lbl+'</div>';
  el.innerHTML+='<div style="font-size:8px;font-family:var(--fm);color:var(--t3)">ADX:'+marketRegime.adx+' BBW:'+marketRegime.bbw+'% Vol:'+marketRegime.volRatio+'x</div></div></div>';
}

/* ═══ P2-5: SOCIAL SENTIMENT (from available data) ═══ */
function calcSocialSentiment(){
  var ar=lang==='ar';
  var allCoins=Object.values(T);var total=allCoins.length;if(!total)return{score:50,label:'--',col:'var(--t3)'};
  var bullPct=Math.round(allCoins.filter(function(x){return x.c>0}).length/total*100);
  var avgChg=allCoins.reduce(function(s,x){return s+x.c},0)/total;
  var megaVol=allCoins.filter(function(x){return x.v>1e9}).length;
  var score=50;
  score+=Math.round((bullPct-50)*0.3);
  score+=Math.round(avgChg*2);
  if(fgValue>70)score+=10;else if(fgValue<30)score-=10;
  score+=megaVol*2;
  score=Math.max(0,Math.min(100,score));
  var label,col;
  if(score>=70){label=ar?'🟢 طمع — حماس عالي':'🟢 Greedy — High enthusiasm';col='var(--up)'}
  else if(score>=55){label=ar?'🟢 إيجابي':'🟢 Positive';col='var(--up)'}
  else if(score>=45){label=ar?'🟡 محايد':'🟡 Neutral';col='var(--warn)'}
  else if(score>=30){label=ar?'🔴 سلبي':'🔴 Negative';col='var(--dn)'}
  else{label=ar?'🔴 خوف شديد':'🔴 Extreme fear';col='var(--dn)'}
  return{score:score,label:label,col:col,bullPct:bullPct,avgChg:+avgChg.toFixed(1),megaVol:megaVol};
}
...
/* ═══ P3-1: AI TRADE JOURNAL ═══ */
function generateTradeJournal(){
  var ar=lang==='ar';
  var closed=activeTrades.filter(function(t){return t.status==='CLOSED'}).slice(-20);
  if(!closed.length)return ar?'لا صفقات مغلقة بعد — ابدأ التداول!':'No closed trades yet — start trading!';
  var wins=closed.filter(function(t){return t.finalPnl>=0}).length;
  var losses=closed.length-wins;
  var totalPnl=closed.reduce(function(s,t){return s+(t.finalPnl||0)},0);
  var avgWin=wins>0?closed.filter(function(t){return t.finalPnl>=0}).reduce(function(s,t){return s+(t.finalPnl||0)},0)/wins:0;
  var avgLoss=losses>0?Math.abs(closed.filter(function(t){return t.finalPnl<0}).reduce(function(s,t){return s+(t.finalPnl||0)},0)/losses):0;
  var bestTrade=closed.reduce(function(best,t){return(t.finalPnl||0)>(best.finalPnl||0)?t:best},closed[0]);
  var worstTrade=closed.reduce(function(worst,t){return(t.finalPnl||0)<(worst.finalPnl||0)?t:worst},closed[0]);
  ...
  h+='📈 '+(ar?'أفضل صفقة: ':'Best: ')+bestTrade.sym+' +'+(bestTrade.finalPnl||0).toFixed(1)+'%<br>';
  h+='📉 '+(ar?'أسوأ صفقة: ':'Worst: ')+worstTrade.sym+' '+(worstTrade.finalPnl||0).toFixed(1)+'%<br>';
  if(avgLoss>0){var pf=avgWin/avgLoss;h+='⚖️ Profit Factor: <b style="color:'+(pf>1?'var(--up)':'var(--dn)')+'">'+pf.toFixed(1)+'x</b><br>'}
  if(avgWin<avgLoss*0.8){h+='💡 '+(ar?'نصيحة: أرباحك صغيرة مقارنة بخسائرك — وسّع الأهداف أو ضيّق الوقف':'Tip: Wins smaller than losses — widen targets or tighten stops')+'<br>'}
  if(losses>wins*2){h+='💡 '+(ar?'نصيحة: الكثير من الخسائر — ركّز على إشارات ULTRA فقط':'Tip: Too many losses — focus on ULTRA signals only')+'<br>'}
  h+='</div>';
  return h;
}

/* ═══ P3-2: AI MARKET BRIEFING ═══ */
function generateMarketBriefing(){
  var ar=lang==='ar';
  var btc=T.BTC;var eth=T.ETH;
  if(!btc)return'';
  var regime=getRegimeLabel();
  var sentiment=calcSocialSentiment();
  var whaleCount=Object.keys(whaleWaves).filter(function(s){return whaleWaves[s]&&whaleWaves[s].engine&&whaleWaves[s].engine.confidence>=40}).length;
  var frDanger=Object.values(FR).filter(function(f){return f.rate>0.05}).length;
  var h='<div style="font-weight:800;font-size:13px;color:var(--t0);margin-bottom:8px">'+(ar?'🤖 تقرير السوق الذكي':'🤖 AI Market Briefing')+'</div>';
  h+='<div style="font-size:8px;color:var(--t3);margin-bottom:8px">'+new Date().toLocaleTimeString('en',{hour:'2-digit',minute:'2-digit'})+' — '+(ar?'تحديث تلقائي كل 4 ساعات':'Auto-update every 4h')+'</div>';
  h+='<div style="padding:10px;background:var(--bg2);border-radius:10px;font-size:11px;color:var(--t1);line-height:1.7">';
  /* BTC summary */
  h+='<b>₿ BTC:</b> '+fP(btc.p)+' ('+(btc.c>=0?'+':'')+btc.c.toFixed(1)+'%) — ';
  h+=btc.c>3?(ar?'صاعد بقوة 🟢':'Strong rally 🟢'):btc.c>0?(ar?'إيجابي':'Positive'):btc.c>-3?(ar?'تراجع طفيف':'Minor pullback'):(ar?'هبوط ⚠️':'Dropping ⚠️');
  h+='<br>';
  /* ETH */
  if(eth){h+='<b>Ξ ETH:</b> '+fP(eth.p)+' ('+(eth.c>=0?'+':'')+eth.c.toFixed(1)+'%)<br>'}
  /* Regime */
  h+='<br>'+regime.ic+' <b>'+(ar?'حالة السوق:':'Market State:')+'</b> '+regime.lbl+'<br>';
  /* Sentiment */
  h+='<span style="color:'+sentiment.col+'">'+sentiment.label+'</span> — '+sentiment.bullPct+'% '+(ar?'صاعدة':'bullish')+'<br>';
  /* Whales */
  if(whaleCount>0){h+='🐋 '+whaleCount+' '+(ar?'عملة فيها نشاط حيتان':'coins with whale activity')+'<br>'}
  /* FR warnings */
  if(frDanger>0){h+='⚠️ '+frDanger+' '+(ar?'عملة FR خطر':'coins with dangerous FR')+'<br>'}
  /* F&G */
  h+='<br>😱 Fear & Greed: <b style="color:'+(fgValue<30?'var(--dn)':fgValue>60?'var(--up)':'var(--warn)')+'">'+fgValue+'</b> — BTC Dom: <b>'+btcDom.toFixed(1)+'%</b><br>';
  /* Recommendation */
  h+='<br><b>'+(ar?'💡 التوصية:':'💡 Recommendation:')+'</b> ';
  if(btc.c>2&&sentiment.score>60&&whaleCount>2){h+=ar?'سوق صاعد — ابحث عن فرص دخول مع التيار':'Bullish market — look for entries with the trend'}
  else if(btc.c<-3||sentiment.score<35){h+=ar?'سوق هابط — تجنب Long، احمِ رأس مالك':'Bearish — avoid Longs, protect capital'}
  else{h+=ar?'سوق محايد — انتظر إشارة واضحة أو تداول بحذر':'Neutral — wait for clear signal or trade carefully'}
  h+='</div>';
  return h;
}

/* ═══ P3-3: CHART PATTERN RECOGNITION ═══ */
function detectChartPatterns(sym){
  var ar=lang==='ar';var patterns=[];
  var sh=sparkHist[sym];var d=T[sym];
  if(!sh||sh.length<8||!d)return patterns;
  var prices=sh.slice(-12);var n=prices.length;
  /* Double Bottom */
  if(n>=8){
    var low1=Math.min.apply(null,prices.slice(0,Math.floor(n/2)));
    var low2=Math.min.apply(null,prices.slice(Math.floor(n/2)));
    var mid=Math.max.apply(null,prices.slice(Math.floor(n/4),Math.floor(n*3/4)));
    if(Math.abs(low1-low2)/low1<0.02&&mid>low1*1.02){
      patterns.push({name:ar?'قاع مزدوج':'Double Bottom',type:'bullish',conf:72,ic:'📈'});
    }
  }
  /* Higher Highs + Higher Lows (Uptrend) */
  if(n>=6){
    var hh=true;var hl=true;
    for(var i=2;i<n;i+=2){
      if(prices[i]<=prices[i-2])hh=false;
      if(i>0&&prices[i-1]<=prices[i-3])hl=false;
    }
    if(hh&&hl)patterns.push({name:ar?'ترند صاعد':'Uptrend',type:'bullish',conf:68,ic:'📈'});
  }
  /* Lower Highs (Downtrend) */
  if(n>=6){
    var lh=true;
    for(var i=2;i<n;i+=2){if(prices[i]>=prices[i-2])lh=false}
    if(lh)patterns.push({name:ar?'ترند هابط':'Downtrend',type:'bearish',conf:65,ic:'📉'});
  }
  /* Compression (Triangle) — decreasing range */
  if(n>=8){
    var r1=Math.max.apply(null,prices.slice(0,4))-Math.min.apply(null,prices.slice(0,4));
    var r2=Math.max.apply(null,prices.slice(-4))-Math.min.apply(null,prices.slice(-4));
    if(r2<r1*0.6&&r1>0){
      patterns.push({name:ar?'ضغط (مثلث) — انفجار قريب':'Compression — Breakout soon',type:'neutral',conf:70,ic:'🔺'});
    }
  }
  /* Volume climax */
  if(d.v>1e9&&Math.abs(d.c)>5){
    patterns.push({name:ar?'ذروة حجم — نهاية حركة':'Volume Climax — Move ending',type:'reversal',conf:60,ic:'💥'});
  }
  return patterns;
}

/* ═══ P3-4: PREDICTIVE SCORING ═══ */
function getPredictiveScore(sym){
  var d=T[sym];if(!d)return 0;
  var score=50;var W=monitorState?monitorState.weights:DEFAULT_WEIGHTS;
  /* Trend */
  if(d.c>2)score+=5*((W.trend||1)/1);
  else if(d.c<-2)score-=5*((W.trend||1)/1);
  /* Whales */
  var ww=whaleWaves[sym];if(ww&&ww.engine&&ww.engine.confidence>=50)score+=10*((W.whales||1)/1);
  /* FR */
  var fr=FR[sym];if(fr&&fr.rate<-0.02)score+=8*((W.fr||1)/1);
  if(fr&&fr.rate>0.05)score-=8*((W.fr||1)/1);
  /* Volume */
  if(d.v>1e8)score+=5*((W.vol||0.5)/0.5);
  /* Historical coin performance */
  if(monitorState&&monitorState.coinStats&&monitorState.coinStats[sym]){
    var cs=monitorState.coinStats[sym];
    if(cs.total>=3){score+=(cs.rate-50)*0.2}
  }
  /* Market regime */
  if(marketRegime.type==='trending'&&d.c>0)score+=5;
  if(marketRegime.type==='quiet')score-=5;
  return Math.max(0,Math.min(100,Math.round(score)));
}

/* ═══ P3-5: PORTFOLIO RISK AI ═══ */
function analyzePortfolioRisk(){
  var ar=lang==='ar';
  var open=activeTrades.filter(function(t){return t.status==='OPEN'});
  if(!open.length)return{level:'safe',score:0,warnings:[],suggestions:[]};
  var warnings=[];var suggestions=[];var riskScore=0;
  /* Check 1: Too many open trades */
  if(open.length>=4){riskScore+=20;warnings.push(ar?'⚠️ '+open.length+' صفقات مفتوحة — كثير!':'⚠️ '+open.length+' open trades — too many!')}
  /* Check 2: Correlated positions */
  var syms=open.map(function(t){return t.sym});
  var sameDir=open.filter(function(t){var d=T[t.sym];return d&&d.c>0}).length;
  if(sameDir===open.length&&open.length>=3){riskScore+=25;warnings.push(ar?'🔴 كل الصفقات بنفس الاتجاه!':'🔴 All trades same direction!')}
  /* Check 3: BTC correlation risk */
  var btcDown=T.BTC&&T.BTC.c<-3;
  if(btcDown&&open.length>0){riskScore+=15;warnings.push(ar?'📉 BTC هابط — كل الصفقات في خطر':'📉 BTC dropping — all trades at risk')}
  /* Check 4: Underwater positions */
  var underwater=open.filter(function(t){return t.pnl<-3}).length;
  if(underwater>0){riskScore+=underwater*10;warnings.push(ar?'🩸 '+underwater+' صفقة في خسارة > 3%':'🩸 '+underwater+' trades down > 3%')}
  /* Suggestions */
  if(riskScore>=40)suggestions.push(ar?'💡 أقفل أضعف صفقة':'💡 Close weakest trade');
  if(sameDir===open.length&&open.length>=2)suggestions.push(ar?'💡 نوّع — لا تكون كل الصفقات Long':'💡 Diversify — not all Long');
  if(btcDown)suggestions.push(ar?'💡 ضيّق الوقف أو اخرج':'💡 Tighten stops or exit');
  var level=riskScore>=50?'danger':riskScore>=25?'caution':'safe';
  return{level:level,score:Math.min(100,riskScore),warnings:warnings,suggestions:suggestions};
}

/* ═══ RENDER ALL NEW FEATURES ON DASHBOARD ═══ */
function renderNewFeatures(){
  try{renderLivePnL()}catch(e){}
  try{renderRegimeBadge()}catch(e){}
  try{renderCorrelationMap()}catch(e){}
  try{renderLiqHeatmapCard()}catch(e){}
  try{renderOrderFlow()}catch(e){}
  try{renderArbitrage()}catch(e){}
  /* AI Briefing */
  try{
    var briefEl=document.getElementById('aiBriefing');
    if(briefEl)briefEl.innerHTML=generateMarketBriefing();
  }catch(e){}
  /* Trade Journal */
  try{
    var journalEl=document.getElementById('tradeJournal');
    if(journalEl)journalEl.innerHTML=generateTradeJournal();
  }catch(e){}
  /* Portfolio Risk */
  try{
    var riskEl=document.getElementById('portRisk');if(riskEl){
      var risk=analyzePortfolioRisk();
      var col=risk.level==='danger'?'var(--dn)':risk.level==='caution'?'var(--warn)':'var(--up)';
      var h='<div style="display:flex;align-items:center;gap:8px;padding:6px 10px;background:'+(risk.level==='danger'?'var(--dd)':risk.level==='caution'?'var(--wd)':'var(--ud)')+';border-radius:8px">';
      h+='<div style="font-family:var(--fm);font-size:18px;font-weight:800;color:'+col+'">'+risk.score+'%</div>';
      h+='<div style="flex:1"><div style="font-size:10px;font-weight:700;color:'+col+'">'+(lang==='ar'?'مخاطر المحفظة':'Portfolio Risk')+'</div>';
      if(risk.warnings.length)h+='<div style="font-size:8px;color:var(--t2)">'+risk.warnings[0]+'</div>';
      if(risk.suggestions.length)h+='<div style="font-size:8px;color:var(--neon)">'+risk.suggestions[0]+'</div>';
      h+='</div></div>';
      riskEl.innerHTML=h;
    }
  }catch(e){}
  /* Sentiment */
  try{
    var sentEl=document.getElementById('sentimentBadge');if(sentEl){
      var s=calcSocialSentiment();
      sentEl.innerHTML='<div style="display:flex;align-items:center;gap:6px;padding:4px 8px;background:var(--bg2);border-radius:6px;font-size:10px"><span style="font-weight:700;color:'+s.col+'">'+s.label+'</span><span style="font-family:var(--fm);font-size:9px;color:var(--t3)">'+s.score+'/100</span></div>';
    }
  }catch(e){}
  /* Pattern Detection on top coins */
  try{
    var patEl=document.getElementById('patternList');if(patEl){
      var allPats=[];
      ['BTC','ETH','SOL','BNB','XRP'].forEach(function(sym){
        var pats=detectChartPatterns(sym);
        pats.forEach(function(p){allPats.push({sym:sym,pattern:p})});
      });
      if(allPats.length){
        var ph='';allPats.slice(0,5).forEach(function(item){
          var col=item.pattern.type==='bullish'?'var(--up)':item.pattern.type==='bearish'?'var(--dn)':'var(--warn)';
          ph+='<div style="display:flex;align-items:center;gap:6px;padding:4px 0;border-bottom:1px solid var(--bdr);font-size:10px">';
          ph+='<span style="font-weight:700;min-width:32px">'+item.sym+'</span>';
          ph+='<span style="font-size:12px">'+item.pattern.ic+'</span>';
          ph+='<span style="flex:1;color:'+col+'">'+item.pattern.name+'</span>';
          ph+='<span style="font-family:var(--fm);font-weight:700;color:'+col+'">'+item.pattern.conf+'%</span></div>';
        });
        patEl.innerHTML=ph;
      }else{patEl.innerHTML='<div style="font-size:9px;color:var(--t3);text-align:center">'+(lang==='ar'?'لا أنماط واضحة':'No clear patterns')+'</div>'}
    }
  }catch(e){}
  /* Predictive scores */
  try{
    var predEl=document.getElementById('predScores');if(predEl){
      var coins=['BTC','ETH','SOL','BNB','XRP'];
      var ph='';coins.forEach(function(sym){
        var sc=getPredictiveScore(sym);
        var col=sc>=65?'var(--up)':sc>=45?'var(--warn)':'var(--dn)';
        ph+='<div style="display:flex;align-items:center;gap:4px;font-size:9px;padding:2px 0">';
        ph+='<span style="font-weight:700;min-width:28px">'+sym+'</span>';
        ph+='<div style="flex:1;height:4px;background:var(--bg2);border-radius:2px;overflow:hidden"><div style="width:'+sc+'%;height:100%;background:'+col+';border-radius:2px"></div></div>';
        ph+='<span style="font-family:var(--fm);font-weight:700;color:'+col+';min-width:28px">'+sc+'</span></div>';
      });
      predEl.innerHTML=ph;
    }
  }catch(e){}
...
/* ═══ BOOT NEW FEATURES — call from init() ═══ */
function bootNewFeatures(){
  /* Market Regime — detect on start, then every 15 min */
  setTimeout(function(){try{detectMarketRegime()}catch(e){}},8000);
  setInterval(function(){try{detectMarketRegime()}catch(e){}},900000);
  /* Render new dashboard features every 30s */
  setTimeout(function(){try{renderNewFeatures()}catch(e){}},12000);
  setInterval(function(){try{renderNewFeatures()}catch(e){}},30000);
  /* Smart alerts every 60s */
  setInterval(function(){try{checkSmartAlerts()}catch(e){}},60000);
