...
async function runScannerOnServer() {
  ...
  /* Market regime is classified inside runScannerPass (it drives weight-
     profile selection there when SCANNER_REGIME_ADAPTIVE is on). Surface the
...
/* Per-tag scanner win-rate (Phase 3.1). Aggregates the rolling
   scanner-history into a tag → {count, wins, winRate, avgGain,
   bestSignal, worstSignal} map so the engineering team can see
   which tags actually predict good outcomes. Gated by
   SCANNER_TAG_STATS_ENABLED so the endpoint can be removed
   without redeploy. Data is generated by the P1.0b schema
...
app.get('/api/scanner/tag-stats', (req, res) => {
  ...
  const minSamples = Number.isFinite(minRaw) && minRaw >= 1 && minRaw <= 100 ? minRaw : 3;
  const stats = scannerTagStats.computeTagStats(cache.scannerHistory || [], {
    daysBack: days,
    minSamples,
    now: Date.now(),
  });
